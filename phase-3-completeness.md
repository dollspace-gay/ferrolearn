...
- REQ-18: Remaining toy datasets (Digits, Linnerud, Olivetti Faces) in `ferrolearn-datasets` ### Infrastructure
...
- REQ-25: `no_std` support for `ferrolearn-core` traits and `ferrolearn-metrics` (behind default `std` feature flag; disabling `std` removes I/O and thread-dependent code) ### Verification
