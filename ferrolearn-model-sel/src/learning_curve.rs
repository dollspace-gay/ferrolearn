...
//! | REQ-DEFAULTS (default train_sizes / cv / scoring) | NOT-STARTED | `train_sizes`/`cv`/`scoring` all mandatory; no `linspace(0.1,1,5)` default, and this driver does not yet consume `check_cv`/`check_scoring` defaults (`:1730`, `:1908`, `:1912`). Blocker #1769. |
