...
//! f32, whereas scikit-learn 1.5.2 computes it via `np.ma.mean` in
//! float64-precision (`sklearn/impute/_base.py:497-501`:
//! `mean_masked = np.ma.mean(masked_X, axis=0)`), storing a float64
//! `statistics_`. On `transform`, sklearn fills the missing entries from that
//! float64 statistic, rounding to f32 only at the assignment into the float32
//! output array (`_base.py:625-636`). For a column whose naive sequential f32
//! accumulation loses precision, ferrolearn's fill value diverges from
//! sklearn's by many f32 ULPs (NOT a 1-ULP rounding difference).
//!
...
use ndarray::{Array2, Axis, concatenate, array};
...
#[test]
#[ignore = "divergence: SimpleImputer<f32> mean accumulates in f32, sklearn uses float64 np.ma.mean; tracking #2308"]
fn divergence_f32_mean_transform_fill_float64_accumulation() {
    ...
    let out = fitted.transform(&x_transform).expect("transform should succeed");
