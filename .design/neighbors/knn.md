...
- REQ-6: **shared k-NN search value (R-DEV-1/3).** Mirror the `KNeighborsMixin`
  k-NN kernel both predict paths consume: nearest-first `(distances, indices)`.
  ferrolearn `pub(crate) fn kneighbors_impl` / `pub fn kneighbors` value-matches.
...
| REQ-6 (shared k-NN search value) | SHIPPED | impl `pub(crate) fn kneighbors_impl` + `pub fn kneighbors` (both fitted types) → per-row `fn find_neighbors` (KdTree/BallTree/`brute_force_knn`) fills `(n_queries,k)` nearest-first `(distances, indices)`, the kernel both `predict` paths consume. Non-test consumers: `fn predict` (clf + reg) call `find_neighbors`; the `kneighbors` method is pub on the bound types. Same Euclidean k-NN kernel whose oracle parity is established in `.design/neighbors/nearest_neighbors.md` (AC-1) and `balltree.md` (AC-1). Backs AC-1/2/4. |
