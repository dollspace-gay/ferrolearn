...
| REQ-4 (metric="manhattan" median) | NOT-STARTED | open prereq blocker #803. No `metric` field exists; centroid is always the **mean**. sklearn `metric="manhattan"` uses the feature-wise **median** `np.median(X[mask], axis=0)` (`_nearest_centroid.py:164-167`) and manhattan-metric `predict` (`:218`). Pin (AC-5): manhattan `centroids_==[[1.0,0.0],[5.5,5.0]]` vs euclidean mean `[[3.6666...,0.0],[36.8333...,5.0]]`; ferro inexpressible. |
