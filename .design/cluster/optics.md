...
| REQ-2 (`ordering_` traversal value-parity) | SHIPPED | impl public `compute_optics_graph` + `Fit::fit` select the next seed by LINEAR ARGMIN over all unprocessed reachability with smallest-index tie-break (single pool, no heap), matching sklearn `compute_optics_graph` (`_optics.py:638-659`, `:53` "we do not employ a heap"). The prior `BinaryHeap` traversal diverged on tie-prone data. Guards: `green_compute_optics_graph_public_helper_docstring`, `green_ordering_small10` (sklearn `[0,1,3,9,5,6,7,8,2,4]`; was `[…5,7,8,6…]`) + `green_ordering_three_blobs`/`green_ordering_docstring`. Fixed #1080. |
...
| REQ-5 (`labels_` VALUE parity) | SHIPPED | impl: `Fit::fit` Xi arm → public `fn cluster_optics_xi` → `fn xi_cluster` → `fn extract_xi_labels`, a faithful port of sklearn `cluster_optics_xi`/`_xi_cluster`/`_extract_xi_labels` (`_optics.py:810-1201`): appended `+inf` sentinel, IEEE-754 `ratio` reproducing numpy `errstate(invalid="ignore")`, paper-corrected steep inequalities, Definition-11 criteria 1-4, leaf-only assign + `labels[ordering]` scatter. The prior `fn xi_cluster_extraction`/`fn filter_small_clusters` (dropped in-Xi size + post-filter) were REPLACED. Consumer: `Fit::fit` Xi arm + `FittedOPTICS::extract_clusters`, reachable via the crate re-export. Guards (live-oracle): `green_xi_labels_hierarchy_doctest`/`_three_blobs`/`_small10`, `green_cluster_optics_xi_public_helper_doctest`, `green_xi_min_cluster_size_inside_loop`, `green_xi_varied_threshold_small10`, `green_xi_predecessor_correction_toggle_small10` in `tests/divergence_optics.rs` (bit-exact `labels_`, f64). |
...
| REQ-7 (Xi `min_cluster_size` criterion 3.a) | SHIPPED | impl: `fn xi_cluster` applies `if c_end + 1 - c_start < min_cluster_size { continue; }` INSIDE the steep-up loop, matching sklearn `_optics.py:1154-1156` (NOT a post-filter — the old `fn filter_small_clusters` was removed). `min_cluster_size` resolves `None → min_samples` in `fn cluster_optics_xi` (`:902-903`). Guard: `green_xi_min_cluster_size_inside_loop` — `blobs44` ms3: the 4-point blob survives at the default but is dropped at `min_cluster_size=10`, changing BOTH `labels_` and `cluster_hierarchy_` like sklearn (proves the cut is mid-loop, not post-hoc). |
| REQ-8 (`cluster_hierarchy_` attribute) | SHIPPED | impl: `FittedOPTICS.cluster_hierarchy_: Array2<i64>` (`(n_clusters,2)` `[start,end]` plot-order intervals), accessor `fn cluster_hierarchy`. Built in `fn cluster_optics_xi` from `fn xi_cluster`'s intervals (`np.array(clusters)`, `_optics.py:1172`), set on the `Xi` arm of `fit` (`:373`); `Dbscan` arm leaves it empty `(0,2)`. Order = `_xi_cluster` discovery order (U_clusters reversed); `cluster_optics_xi` does NOT re-sort (matches the docstring's `(end,-start)` on these fixtures, verified vs oracle). Guards: `green_xi_labels_hierarchy_doctest` (`[[0,2],[3,5],[0,5]]`, `len(hierarchy)=3 > 2=n_unique` nested-kept), `_three_blobs`, `_small10`, `green_dbscan_cluster_hierarchy_empty`. |
| REQ-9 (`predecessor_correction` toggle) | SHIPPED | impl: `OPTICS<F>.predecessor_correction: bool` (default `true`, builder `fn with_predecessor_correction`), threaded `Fit::fit` → `fn cluster_optics_xi` → `fn xi_cluster`, which calls `fn correct_predecessor` only when set — matching sklearn `:277`/`:1147-1150`. Guard: `green_xi_predecessor_correction_toggle_small10` (off → `[4,7]→[4,8]`, point 2 labelled, bit-exact). |
