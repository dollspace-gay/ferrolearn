...
`FeatureAgglomeration` contract end-to-end** — the feature partition matches sklearn,
but the integer label assignment (`labels_`, REQ-3) is permuted, which cascades into
the column ordering of `transform` (REQ-1/REQ-2). Every REQ is NOT-STARTED. Suggested
blocker numbers — the director creates the real issues; #937 is this doc's crosslink
tracking issue, reused for the core upstream label-ordering divergence (REQ-3).

| REQ | Status | Evidence |
|---|---|---|
| REQ-1 (`transform` mean VALUE) | NOT-STARTED | open prereq blocker **#937** (depends on REQ-3 #940). impl `fn transform` (PoolingFunc::Mean) computes per-cluster mean (`sum/count`) into `(n_samples, n_clusters)`, mirroring `AgglomerationTransform.transform` (`_feature_agglomeration.py:51-57`). The pooling ARITHMETIC is correct, but output **column order diverges** from sklearn (which orders by `np.unique(labels_)`, `:62`) because the underlying label index is permuted (REQ-3). Pin (AC-2): `transform(X)[0]` sklearn `[1.05, 9.05, 5.05]` vs ferrolearn `[1.05, 5.05, 9.05]` — element `[0,1]` `9.05` vs `5.05`. Equal only as an unordered column set. Unblocks once REQ-3 (label parity) lands. |
| REQ-2 (`transform` max VALUE) | NOT-STARTED | open prereq blocker **#937** (depends on REQ-3 #940). impl `fn transform` (PoolingFunc::Max) takes per-cluster max, mirroring the general pooling path (`_feature_agglomeration.py:58-63`, `np.max` callable). Per-cluster max is correct; column order diverges identically (AC-3): sklearn `transform(X)[0]=[1.1, 9.1, 5.1]` vs ferrolearn `[1.1, 5.1, 9.1]`. Gated on REQ-3. |
| REQ-3 (`labels_` feature-cluster VALUE) | NOT-STARTED | open prereq blocker **#940** (owned by the `agglomerative.rs` unit; the root cause). `fit` delegates to `AgglomerativeClustering::new(n_clusters).with_linkage(...)` on `X.T` (`fn fit`), storing `feature_labels_`. The PARTITION matches sklearn (`{0,1}/{2,3}/{4,5}`, Probe 1) but the integer labels are PERMUTED: sklearn `_hc_cut` (`_agglomerative.py:1099`) + `np.searchsorted(np.unique(labels), labels)` (`:1105`) → `[0,0,2,2,1,1]`; ferrolearn `agglomerative.rs::agglomerate` re-labels by `active`-slot order → `[0,0,1,1,2,2]` (AC-1). **The critic should pin REQ-3 FIRST** — fixing the `_hc_cut`/`searchsorted` label-numbering convention in `agglomerative.rs` is the single root cause; REQ-1/REQ-2 (transform column order) unblock automatically once it lands. |
| REQ-4 (validation guards + ensure_min_features=2 + error ABI) | NOT-STARTED | open prereq blocker **#941**. ferrolearn `fn fit` rejects `n_clusters==0`, `n_features < n_clusters`, empty X — but has **no `ensure_min_features=2`** (sklearn `:1338` rejects a single feature; ferrolearn ACCEPTS it, AC-4) and raises `FerroError::InvalidParameter`/`InsufficientSamples`, not the sklearn `ValueError`/`InvalidParameterError` ABI (R-DEV-2). |
| REQ-5 (linkage variants ward/complete/average/single) | NOT-STARTED | open prereq blocker **#940** (shares the REQ-3 root cause). `fn map_linkage` maps all four `AgglomerativeLinkage` variants to `Linkage`, delegated to `AgglomerativeClustering` (mirroring `_TREE_BUILDERS`, `_agglomerative.py:1290`). The partition matches on the fixture across all four (Probe 1), but `labels_`/`transform` VALUE is gated on REQ-3 (label-index parity). The variants themselves are wired, but VALUE parity is not established. |
