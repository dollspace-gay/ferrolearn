...
| REQ-8 (`components_` fitted attribute) | NOT-STARTED | open prereq blocker **#950**. sklearn `components_ = X[core_sample_indices_].copy()` (`_dbscan.py:441-446`; Probe 6: shape `(2,2)` for 2 core samples). `FittedDBSCAN` exposes `core_sample_indices()` but no `components_` accessor and does not retain `X`. Missing attribute. |
