...
- REQ-PCA-SCORE: `ferrolearn.PCA` exposes the `score(X, y=None)` (float) and
  `score_samples(X)` `(n_samples,)` methods, returning the average / per-sample
  Gaussian log-likelihood under the probabilistic-PCA model
  (`_pca.py:805-853`). [SHIPPED #2099 — the binding surfaces
  `FittedPCA::score_samples` (`ferrolearn-decomp/src/pca.rs:484`) / `FittedPCA::score`
  (`pca.rs:533`) (downstream prereq `ferrolearn-decomp` REQ-15 #1507 SHIPPED,
  matching the live oracle to 1e-6), wrapped by `_transformers.py::PCA.score`/
  `score_samples`.]
- REQ-PCA-COVARIANCE-PRECISION: `ferrolearn.PCA` exposes the `get_covariance()`
  `(n_features, n_features)` and `get_precision()` `(n_features, n_features)`
  methods, returning the generative probabilistic-PCA model's data covariance and
  its inverse (`_base.py:30-101`). Neither takes an `X` argument. [SHIPPED #2100 —
  the binding surfaces `FittedPCA::get_covariance` (`ferrolearn-decomp/src/pca.rs:328`,
  infallible) / `FittedPCA::get_precision` (`pca.rs:390`, fallible) (downstream
  prereq `ferrolearn-decomp` REQ-14 #1505 SHIPPED, matching the live oracle to
  1e-6), wrapped by `_transformers.py::PCA.get_covariance`/`get_precision`.]
...
- AC-PCA-VALUE-PARITY (REQ-PCA-VALUE-PARITY): the default-solver arrays match
  ...
  REQ-12 #1503).
- AC-PCA-SCORE (REQ-PCA-SCORE): SHIPPED #2099. sklearn oracle on the rank>2
  fixture (so the discarded tail eigenvalue makes the `get_precision`-based
  log-likelihood non-trivial)
  `cd /tmp && python3 -c "import numpy as np; from sklearn.decomposition import PCA; X=np.array([[1.,2.,3.],[4.,5.,7.],[2.,0.,1.],[8.,6.,5.],[3.,3.,2.],[0.,1.,4.]]); m=PCA(2).fit(X); print(m.score(X)); print(m.score_samples(X).tolist())"`
  → `-5.358925927374854` / `[-4.6972115227, -5.3570669702, -5.8335548795,
  -5.7184482497, -5.4976041344, -5.0496698077]`. `ferrolearn.PCA(n_components=2).fit(X)`
  now exposes `score(X)` matching the oracle to <1e-9 and `score_samples(X)`
  element-wise to <1e-9 (via `RsPCA::score`/`score_samples` over
  `FittedPCA::score()`/`score_samples()`, wrapped by `_transformers.py::PCA.score`/
  `score_samples`). Guard
  `tests/divergence_transformers.py::test_pca_score_and_score_samples_match_sklearn`.
- AC-PCA-COVARIANCE-PRECISION (REQ-PCA-COVARIANCE-PRECISION): SHIPPED #2100.
  sklearn oracle on the rank>2 fixture (so the discarded tail eigenvalue makes
  the generative-model covariance/precision non-trivial)
  `cd /tmp && python3 -c "import numpy as np; from sklearn.decomposition import PCA; X=np.array([[1.,2.,3.],[4.,5.,7.],[2.,0.,1.],[8.,6.,5.],[3.,3.,2.],[0.,1.,4.]]); m=PCA(2).fit(X); print(m.get_covariance()[0].tolist()); print(m.get_precision()[0].tolist())"`
  → `[8.0, 5.6, 2.6]` / `[0.7432854466, -1.0993129294, 0.4653341661]`.
  `ferrolearn.PCA(n_components=2).fit(X)` now exposes `get_covariance()`/
  `get_precision()` matching the oracle element-wise to <1e-9 (via
  `RsPCA::get_covariance`/`get_precision` over `FittedPCA::get_covariance`/
  `get_precision`, wrapped by `_transformers.py::PCA.get_covariance`/
  `get_precision`). Guard
  `tests/divergence_transformers.py::test_pca_get_covariance_and_precision_match_sklearn`.
...
| REQ-PCA-NCOMP-POSITIONAL (n_components positional ABI) | SHIPPED (#2035) | `_transformers.py::PCA.__init__(self, n_components=None, *, whiten=False)` places `n_components` before the `*`, so `ferrolearn.PCA(2).n_components == 2` matching sklearn `__init__(self, n_components=None, *, ...)` (`_pca.py:407-409`). Guard `tests/divergence_transformers.py::test_red_pca_n_components_positional`. |
| REQ-PCA-NCOMP-DEFAULT-NONE (n_components default None) | SHIPPED (#2036) | wrapper default `n_components=None` stored verbatim (`PCA().n_components is None`, `_pca.py:409`), resolved at fit time via the `_resolve_n_components` classifier before constructing `_RsPCA`; the live oracle `PCA().fit(X).components_.shape` → `(min(n,p), n_features)`. Guard `tests/divergence_transformers.py::test_red_pca_n_components_default_none`. |
| REQ-PCA-NCOMP-FLOAT-RATIO (n_components int/float-ratio/None all threaded) | SHIPPED (#2109) | the downstream `PCA` ships THREE constructors over `NComponents<F>` (`ferrolearn-decomp/src/pca.rs`, REQ-13a): `PCA::new(usize)` → `NComponents::Count`, `PCA::with_variance_ratio(f64)` → `NComponents::Ratio` (resolves the count AFTER the eigendecomposition by `searchsorted(ratio_cumsum, r, "right")+1`, mirroring sklearn's float branch `_pca.py:659-681`), `PCA::auto()` → `NComponents::Auto` (= `min(n_samples, n_features)`, `_pca.py:523-525`). The binding `RsPCA` now carries a resolved spec `count: Option<usize>` + `ratio: Option<f64>` (was a bare `usize`) and `RsPCA::fit` dispatches `(_, Some(r)) => with_variance_ratio(r)` / `(Some(c), None) => new(c)` / `(None, None) => auto()`, mapping `FerroError -> PyValueError` so an out-of-range ratio surfaces as `ValueError`. Non-test consumer: `_transformers.py::PCA` — the module-level `_resolve_n_components(nc)` helper classifies the verbatim ctor `n_components` (None → `(None,None)` Auto; int not-bool → `(int(nc),None)` Count; float in `(0,1)` → `(None,float(nc))` Ratio; else → `ValueError`) into the binding's `(count, ratio)` kwargs at FIT time (so `PCA(n_components=2.0)` constructs but `.fit()` raises, matching sklearn's `_validate_params`-at-fit, `_pca.py:386-391`), called from `PCA.fit`/`__setstate__`/`_ensure_rs`. Live oracle (R-CHAR-3) `X=[[1,2,3],[4,5,7],[2,0,1],[8,6,5],[3,3,2],[0,1,4]]`: `PCA(n_components=0.95)` → `n_components_=2`; `PCA(0.5)` → `1`; `PCA(0.999)` → `3`; `PCA(2.0).fit` → `ValueError`. Guards `tests/divergence_transformers.py::test_red_pca_n_components_float_ratio_matches_sklearn` + `::test_pca_n_components_float_ratio_variants_match_sklearn`. |
| REQ-PCA-WHITEN (whiten ctor param) | SHIPPED (#2098) | the downstream prereq already SHIPPED (`ferrolearn-decomp` REQ-11 #1502, `PCA::with_whiten` at `pca.rs:180`), so the binding now threads the `whiten` constructor param. impl: `RsPCA` carries `whiten: bool` (`#[pyo3(signature = (n_components=2, whiten=false))]`) and `fit` builds `PCA::<f64>::new(self.n_components).with_whiten(self.whiten)` (`transformers.rs::RsPCA`). The whitening math itself is `PCA::with_whiten` — `Transform::transform` divides each projected column `j` by `sqrt(explained_variance_[j])` (`sklearn/decomposition/_base.py:157-165`), `inverse_transform` re-multiplies (`:192-196`). Non-test consumer: `_transformers.py::PCA.__init__(self, n_components=None, *, whiten=False)` stores `self.whiten` and `PCA.fit` passes `whiten=self.whiten` into `_RsPCA(...)`; `__setstate__` rebuilds with `whiten=getattr(self, "whiten", False)`. Live oracle (R-CHAR-3) `X=[[1,2,3],[4,5,7],[2,0,1],[8,6,5],[3,3,2],[0,1,4]]`: `PCA(n_components=2, whiten=True).fit_transform(X)` matches sklearn element-wise to 1e-6; `whiten=False` (default) is byte-identical to the non-whitened path. Guard `tests/divergence_transformers.py::test_pca_whiten_transform_matches_sklearn`. |
| REQ-PCA-PARAMS (copy/svd_solver/tol/iterated_power/n_oversamples/power_iteration_normalizer/random_state) | NOT-STARTED | open prereq blockers #1503 (`svd_solver`, `ferrolearn-decomp` REQ-12), #1509 (`tol`/`iterated_power`/`n_oversamples`/`power_iteration_normalizer`/`random_state`/`copy`, REQ-17). sklearn `_pca.py:407-423`/`:393-405`. ferrolearn `_transformers.py::PCA.__init__` now exposes `n_components`+`whiten`; the remaining ctor params are absent on `RsPCA::new`. The default `svd_solver` behavior MATCHES (covariance-eigh), so only the param surface + non-default paths are missing — owned downstream; the binding cannot expose what the library lacks. (`whiten` is now SHIPPED — see REQ-PCA-WHITEN.) |
...
| REQ-PCA-SCORE (score + score_samples Gaussian log-likelihood) | SHIPPED (#2099) | the downstream prereq already SHIPPED (`ferrolearn-decomp` REQ-15 #1507), so the binding now surfaces both. impl: `RsPCA::score_samples` getter-style method marshals `FittedPCA::score_samples()` (`ferrolearn-decomp/src/pca.rs:484` — `-0.5*sum(Xr*(Xr@precision),1) - 0.5*(n_features*log(2pi) - logdet(precision))`, `precision = get_precision()`) to a `(n_samples,)` numpy array; `RsPCA::score` marshals `FittedPCA::score()` (`pca.rs:533` — the mean of `score_samples`) to `f64`; both map `FerroError -> PyValueError`. Mirrors sklearn `PCA.score_samples`/`PCA.score` (`sklearn/decomposition/_pca.py:805-853`). Non-test consumer: `_transformers.py::PCA.score_samples`/`::PCA.score` (`check_is_fitted` + `_validate_data(reset=False, dtype="float64")` + `_ensure_f64`, rebuilding `_rs` via `_ensure_rs` the same way `__setstate__` does if absent). Live oracle (R-CHAR-3) `X=[[1,2,3],[4,5,7],[2,0,1],[8,6,5],[3,3,2],[0,1,4]]`, `PCA(n_components=2)`: `score(X)=-5.358925927374854`, `score_samples(X)=[-4.6972115,-5.3570670,-5.8335549,-5.7184482,-5.4976041,-5.0496698]` — match to <1e-9. Guard `tests/divergence_transformers.py::test_pca_score_and_score_samples_match_sklearn`. |
| REQ-PCA-COVARIANCE-PRECISION (get_covariance + get_precision) | SHIPPED (#2100) | the downstream prereq already SHIPPED (`ferrolearn-decomp` REQ-14 #1505), so the binding now surfaces both — the last two unsurfaced `FittedPCA` methods. impl: `RsPCA::get_covariance` marshals `FittedPCA::get_covariance` (`ferrolearn-decomp/src/pca.rs:328` — the INFALLIBLE generative-model data covariance `components_ᵀ·diag(exp_var_diff)·components_ + noise_variance_·I`, `_base.py:30-56`) to a `(n_features, n_features)` numpy array via `ndarray2_to_numpy` (no error mapping); `RsPCA::get_precision` marshals `FittedPCA::get_precision` (`pca.rs:390` — the eigendecomposed inverse, `Result`, `_base.py:58-101`) mapping `FerroError -> PyValueError`. Neither takes an `X`. Non-test consumer: `_transformers.py::PCA.get_covariance`/`::PCA.get_precision` (`check_is_fitted` + `_ensure_rs`; no `_validate_data` — matching sklearn's `get_covariance(self)`/`get_precision(self)` signatures, `_base.py:30`/`:58`). Live oracle (R-CHAR-3) `X=[[1,2,3],[4,5,7],[2,0,1],[8,6,5],[3,3,2],[0,1,4]]`, `PCA(n_components=2)`: `get_covariance()` row 0 = `[8.0, 5.6, 2.6]`; `get_precision()` row 0 = `[0.7432854466, -1.0993129294, 0.4653341661]` — match to <1e-9. Guard `tests/divergence_transformers.py::test_pca_get_covariance_and_precision_match_sklearn`. |
...
- **`RsPCA`** stores a resolved `n_components` spec `count: Option<usize>` +
  `ratio: Option<f64>` plus `whiten: bool` and
  `Option<ferrolearn_decomp::FittedPCA<f64>>`. `new`
  (`#[pyo3(signature = (count=None, ratio=None, whiten=false))]`) stores the spec;
  `fit` dispatches by spec — `(_, Some(r)) => PCA::with_variance_ratio(r)` (the
  float-ratio path, `NComponents::Ratio`), `(Some(c), None) => PCA::new(c)`
  (`NComponents::Count`), `(None, None) => PCA::auto()` (`NComponents::Auto`) —
  `.with_whiten(self.whiten)`, runs it on the coerced X, stores the fitted model
  (mapping `FerroError → PyValueError`, so an out-of-range ratio surfaces as
  `ValueError`); `transform`/`inverse_transform`, the seven getters
  (`components_`/`explained_variance_`/`explained_variance_ratio_`/`mean_`/
  `singular_values_`/`n_components_`/`noise_variance_`) and the
  `score`/`score_samples`/`get_covariance`/`get_precision` methods delegate to
  `FittedPCA`.
...
- **`PCA(TransformerMixin, BaseEstimator)`** —
  `__init__(self, n_components=None, *, whiten=False)`: `n_components` before the
  `*` (REQ-PCA-NCOMP-POSITIONAL) with default `None` (REQ-PCA-NCOMP-DEFAULT-NONE),
  stored verbatim. `fit` classifies it via the module-level
  `_resolve_n_components(nc)` (None → Auto, int → Count, float in `(0,1)` → Ratio,
  else `ValueError`) into the binding's `(count, ratio)` kwargs
  (REQ-PCA-NCOMP-FLOAT-RATIO), constructs `_RsPCA(count=..., ratio=...,
  whiten=self.whiten)`, and copies the five arrays + `n_components_`/
  `noise_variance_` (REQ-PCA-ATTRS) + threads `score`/`score_samples`/
  `get_covariance`/`get_precision`. The remaining `copy`/`svd_solver`/… params are
  absent (REQ-PCA-PARAMS). `__getstate__`/`__setstate__` (and `_ensure_rs`) drop
  and rebuild `_rs` via the same `_resolve_n_components` classifier for pickling.
