...
| REQ-SS-WITH-MEAN-STD (with_mean/with_std honored) | NOT-STARTED | open prereq blocker #1193 (`ferrolearn-preprocess` REQ-6: "always centers+scales; no with_mean/with_std params"). The wrapper carries `with_mean`/`with_std` but `_transformers.py::StandardScaler.fit` ALWAYS builds `_RsStandardScaler()` (no-arg, always centers+scales) and ALWAYS sets `mean_`/`scale_` from the Rust arrays; the `transform`/`inverse_transform` `with_mean`/`with_std` fallback branches are DEAD (gated on `not hasattr(self, "_rs")`, which never holds after `fit`). Live: `StandardScaler(with_mean=False, with_std=False).fit(X)` → `mean_=[1.5,3.5]`, `transform(X) != X`, vs sklearn `mean_=None`/identity (`_data.py:993-995`,`:1064-1067`). Partly wrapper-fixable (the wrapper could set `mean_=None`/`var_=None` and route transform through stored attrs), but full parity needs the downstream `ferrolearn_preprocess::StandardScaler` to accept a with_mean/with_std knob — owned downstream #1193. |
| REQ-SS-COPY (copy ctor param) | NOT-STARTED | open prereq blocker #1193 (`ferrolearn-preprocess` REQ-6 covers `with_mean`/`with_std`/`copy`). sklearn `copy` bool, default `True` (`_data.py:835`/`:829`). The wrapper signature is `(self, *, with_mean=True, with_std=True)` — no `copy`. (The wrappers already copy via `_validate_data`/`_ensure_f64`, so the OBSERVABLE non-mutation holds, but the constructor-ABI param is absent — R-DEV-2.) |
