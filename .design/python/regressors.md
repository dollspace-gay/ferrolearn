...
- REQ-LINREG-PARAMS: `ferrolearn.LinearRegression` exposes the
  ...
  blocker #371; `rank_`/`singular_`/`copy_X`/`n_jobs` REQ-9 blocker #374.]
...
- REQ-RIDGE-VALUE-PARITY: `coef_`/`intercept_` match sklearn array-by-array
  ...
  `alpha` array #385, multi-output #384, and alternate-solver values are owned
  downstream.]
...
| REQ-LASSO-NITER (n_iter_ = real CD count) | NOT-STARTED | open prereq blocker #411 (`ferrolearn-linear` REQ-11: `n_iter_`/`dual_gap_` attrs not tracked). sklearn exposes the ACTUAL CD iteration count (`_coordinate_descent.py:1103`/`:1106`); oracle `Lasso(alpha=0.1).fit(X,y).n_iter_` → `89`. ferrolearn `_regressors.py::Lasso.fit` sets `self.n_iter_ = self.max_iter` — a hardcoded FAKE (live: `n_iter_` → `1000`, never the real count), because the Rust `_RsLasso` exposes NO `n_iter_` getter and `FittedLasso` discards the count. The binding cannot expose an attr the library does not compute — owned downstream. (R-DEV-1/-3: the fitted attribute diverges from sklearn.) |
...
| REQ-ELASTICNET-NITER (n_iter_ = real CD count) | NOT-STARTED | open prereq blocker #417 (`ferrolearn-linear` ElasticNet `n_iter_`/`dual_gap_` attrs not tracked). sklearn exposes the ACTUAL CD iteration count (`_coordinate_descent.py:1103`/`:1106`); oracle `ElasticNet(alpha=0.1).fit(X,y).n_iter_` → `58`. ferrolearn `_regressors.py::ElasticNet.fit` sets `self.n_iter_ = self.max_iter` — a hardcoded FAKE (live: `n_iter_` → `1000`), because the Rust `_RsElasticNet` exposes NO `n_iter_` getter and `FittedElasticNet` discards the count. The binding cannot expose an attr the library does not compute — owned downstream. (R-DEV-1/-3.) |
...
All four expose ONLY `coef_`/`intercept_` getters (plus `fit`/`predict`); there
is no `n_iter_`/`rank_`/`singular_`/`dual_gap_` getter on any of them.
