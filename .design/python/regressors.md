...
5. **value parity off the default path (NOT-STARTED, R-DEV-1; `positive` SHIPPED
   ...
   per-target multi-output); multi-output 2-D `y` (unconstrained) SHIPPED; the
   Lasso/ElasticNet dual-gap stopping criterion remains owned downstream.
...
| REQ-LINREG-PARAMS (copy_X/n_jobs/positive) | NOT-STARTED (positive SHIPPED #2129) | `positive` IS now surfaced (prereq #371 `ferrolearn-linear` REQ-6 NNLS already SHIPPED): `RsLinearRegression::new` takes `positive=false` LAST (`#[pyo3(signature = (fit_intercept=true, positive=false))]`) + threads `.with_positive(self.positive)` into the `fit` builder; `_regressors.py::LinearRegression.__init__(self, *, fit_intercept=True, positive=False)` (keyword-only, matching sklearn `_base.py:574`) passes `positive=self.positive` to the single-output `_RsLinearRegression`, and on the `positive=True` + 2-D-`y` path routes through a PER-TARGET LOOP over the single-output positive binding (sklearn's `optimize.nnls` per-target, `_base.py:649-653`; `rank_`/`singular_` still from the unconstrained-design lstsq SVD, `_base.py:687`). Verification (model B): `tests/divergence_regressors.py::test_linearregression_positive_matches_sklearn` (coef_ >= 0, live NNLS oracle match), `test_all_four_positive_false_unchanged` (byte-identical default guard), `test_positive_param_keyword_only_last` (R-CHAR-3). Still MISSING `copy_X`/`n_jobs` — downstream #374 (the `rank_`/`singular_` half of #374 is SHIPPED — see REQ-LINREG-RANK-SINGULAR). sklearn `_base.py:568-579`. (The wrapper copies via `_validate_data`, so OBSERVABLE non-mutation holds, but the `copy_X`/`n_jobs` ABI params are absent — R-DEV-2.) |
...
| REQ-RIDGE-PARAMS (copy_X/max_iter/tol/solver/positive/random_state) | NOT-STARTED (positive SHIPPED #2129) | `positive` IS now surfaced (prereq #387 `ferrolearn-linear` REQ-9 projected-CD already SHIPPED): `RsRidge::new` takes `positive=false` LAST (`#[pyo3(signature = (alpha=1.0, fit_intercept=true, positive=false))]`) + threads `.with_positive(self.positive)` into the `fit` builder; `_regressors.py::Ridge.__init__(self, alpha=1.0, *, fit_intercept=True, positive=False)` (keyword-only, matching sklearn `_ridge.py:902`) passes `positive=self.positive` to the single-output `_RsRidge`, and on the `positive=True` + 2-D-`y` path routes through a PER-TARGET LOOP over the single-output positive binding (sklearn's L-BFGS-B is per-target-independent, `_ridge.py:923-928`/`:207`; the `_RsRidgeMultiOutput` binding has no positivity support). Verification (model B): `tests/divergence_regressors.py::test_ridge_positive_matches_sklearn`, `test_ridge_positive_multioutput_matches_sklearn` (coef_ >= 0, live oracle match + per-target-stack independence), `test_all_four_positive_false_unchanged`, `test_positive_param_keyword_only_last` (R-CHAR-3). Still MISSING `copy_X`/`max_iter`/`tol`/`solver`/`random_state` — downstream #386/#388/#390. sklearn `_ridge.py:893-912`. |
