...
- REQ-OPTICS-BINDING (#1090): `OPTICS` is bound over `ferrolearn_cluster::OPTICS<f64>`
  / `FittedOPTICS<f64>` (optics.rs REQ-1..11 SHIPPED, bit-exact vs the live sklearn
  1.5.2 oracle). `RsOPTICS` (hand-written `#[pyclass]`, `extras.rs`) takes the
  supported ctor surface `(min_samples=5, max_eps=inf, xi=0.05,
  min_cluster_size=None, cluster_method="xi", eps=None, predecessor_correction=true)`
  (`_optics.py:266-297`), resolves `cluster_method` case-sensitively
  (`"xi"`→`Xi`/`"dbscan"`→`Dbscan`; unknown→`PyValueError`, `_optics.py:251`),
  chains `with_min_cluster_size`/`with_eps` only when `Some`, runs `Fit::fit`, and
  surfaces `labels_`/`reachability_`/`ordering_`/`core_distances_`/`predecessor_`/
  `cluster_hierarchy_` via `#[getter]`s (`_optics.py:175-209`). The
  `_extras.py::OPTICS(_ClusterWrapper)` wrapper mirrors sklearn's full keyword-only
  ctor and rejects the core-unsupported surface (non-Euclidean `metric`/`p!=2`/
  `metric_params`/`algorithm∉{auto,brute}`/float `min_samples` → `NotImplementedError`,
  REQ-10 NOT-STARTED #1088). [SHIPPED — see the REQ table row.]
...
| REQ-OPTICS-BINDING (OPTICS density clusterer + reachability/hierarchy attrs, REQ-12 #1090) | SHIPPED | FIXED #1090. `ferrolearn.OPTICS` mirrors `sklearn.cluster.OPTICS` (`sklearn/cluster/_optics.py:266-297`), exposing the keyword-only constructor `(*, min_samples=5, max_eps=np.inf, metric='minkowski', p=2, metric_params=None, cluster_method='xi', eps=None, xi=0.05, predecessor_correction=True, min_cluster_size=None, algorithm='auto', leaf_size=30, memory=None, n_jobs=None)`, the `ClusterMixin` `fit(X, y=None)`/`fit_predict` surface, and the fitted attrs `labels_`/`reachability_`/`ordering_`/`core_distances_`/`predecessor_`/`cluster_hierarchy_`/`n_features_in_` (`_optics.py:175-209`). Impl: hand-written `#[pyclass(name = "_RsOPTICS")]` in `extras.rs` over `ferrolearn_cluster::OPTICS<f64>`/`FittedOPTICS<f64>` (`new` takes `min_samples: usize`/`max_eps: f64`/`xi: f64`/`min_cluster_size: Option<usize>`/`cluster_method: String`/`eps: Option<f64>`/`predecessor_correction: bool`; `fit(x)` resolves `cluster_method` case-sensitively to `OpticsClusterMethod::{Xi,Dbscan}` — `ferrolearn_cluster::optics::OpticsClusterMethod`, unknown → `PyValueError` matching sklearn's `StrOptions({"xi","dbscan"})` `InvalidParameterError ⊂ ValueError` `_optics.py:251` — then builds `OPTICS::new(min_samples).with_max_eps().with_xi().with_cluster_method().with_predecessor_correction()`, chaining `.with_min_cluster_size()`/`.with_eps()` ONLY when `Some` (a `None` `min_cluster_size` leaves the core default = `min_samples`, `_optics.py:902-903`); `#[getter] labels_`→`PyArray1<i64>` (cast `isize`), `ordering_`→`PyArray1<i64>` (cast `usize`), `reachability_`/`core_distances_`→`PyArray1<f64>` (`ndarray1_to_numpy`, inf preserved), `predecessor_`→`PyArray1<i64>` (already `-1`-sentinel `Array1<i64>`), `cluster_hierarchy_`→`PyArray2<i64>`; no `unwrap`/`expect`/`panic` — not-fitted → `PyRuntimeError`, FerroError → `PyValueError`). The Rust OPTICS core was UNTOUCHED (capability pre-existing/audited, `ferrolearn_cluster` optics.rs REQ-1..11 SHIPPED — `compute_optics_graph`/`cluster_optics_xi`/`cluster_optics_dbscan` bit-exact on f64). The `_extras.py::OPTICS(_ClusterWrapper)` wrapper mirrors sklearn's full keyword-only ctor (storing all 14 params for `get_params`/`clone`), passes the 7 SUPPORTED ones to `_RsOPTICS`, and rejects the core-unsupported surface BEFORE the ABI (`_validate_unsupported`): non-`'minkowski'` `metric`, `p != 2`, `metric_params != None`, `algorithm ∉ {'auto','brute'}`, and a FLOAT `min_samples`/`min_cluster_size` (sklearn's `(0,1]` fraction, `_optics.py:245/906`) raise `NotImplementedError` (REQ-10 NOT-STARTED #1088) rather than silently mis-resolving; `leaf_size`/`memory`/`n_jobs` (no-op knobs) are accept-and-ignore. `fit` sets `labels_`/`reachability_`/`ordering_`/`core_distances_`/`predecessor_`/`n_features_in_` always, and `cluster_hierarchy_` ONLY for `cluster_method='xi'` (sklearn sets it only on the Xi branch, `_optics.py:373`; the dbscan branch leaves it UNSET — the wrapper matches sklearn's `hasattr` contract). **VALUE parity (NOT just structural):** `ordering_`/`labels_`/`predecessor_` integer-EXACT, `core_distances_`/`reachability_` ≈ sklearn (atol 1e-9, inf positions equal), `cluster_hierarchy_` int-equal — verified against the live sklearn 1.5.2 oracle on the `_optics.py:891-896` doctest fixture (`labels_ [0,0,0,1,1,1]`, `cluster_hierarchy_ [[0,2],[3,5],[0,5]]`) and a 3-blob `make_blobs` fixture, across Xi (default) AND `cluster_method='dbscan'`+`eps`, `min_cluster_size` (int), and `xi` variants. Non-test production consumer (R-DEFER-1): `_extras.py::OPTICS` (`_make_rs`/`fit`/inherited `fit_predict` + the attr exposure) + `ferrolearn/__init__.py` re-export (`OPTICS` in the import block + `__all__`) + `lib.rs` `m.add_class::<extras::RsOPTICS>()`. Verification (model B): `tests/test_optics.py` (25 tests, live sklearn 1.5.2 oracle, R-CHAR-3) — full-attr Xi parity (doctest + blobs), `min_cluster_size`/`xi`/`predecessor_correction` variants, `cluster_method='dbscan'`+`eps∈{1,2,5}` labels parity, dbscan-omits-`cluster_hierarchy_` vs Xi-has-it (`hasattr`), validation (`min_samples=1`/`min_samples>n`/unknown `cluster_method` → `ValueError`; float `min_samples`/non-minkowski `metric`/`p=3` → `NotImplementedError`, each first asserting sklearn ACCEPTS it so the gap is real), `get_params` includes the 14 sklearn params, `clone()` round-trip + default `min_samples == 5`, `fit_predict == fit().labels_ ==` sklearn. REQ-10 (float `min_samples` fraction + `metric`/`p`/`algorithm`/`leaf_size`/`n_jobs`/`metric_params`) stays NOT-STARTED in `ferrolearn_cluster` optics.rs (open prereq blocker #1088); the binding honestly rejects the non-default unsupported params rather than silently ignoring. |
