...
| REQ-RF-PREDICT-PROBA (predict_proba surfaced) | NOT-STARTED | blocker issue to be filed by critic (binding-level: neither wrapper nor `_Rs*` exposes it, though the library computes it). sklearn RF exposes `predict_proba` `(n_samples, n_classes)` (`_forest.py:922`, mean of per-tree probas; oracle shape `(6,2)`). `_classifiers.py::RandomForestClassifier` has NO `predict_proba` method and `RsRandomForestClassifier` (`classifiers.rs`) exposes NO `predict_proba` getter — live: `hasattr(ferrolearn.RandomForestClassifier, 'predict_proba')` → `False`. The library `FittedRandomForestClassifier::predict_proba` EXISTS (downstream `ferrolearn-tree` REQ-5 SHIPPED) — bind + surface it. |
