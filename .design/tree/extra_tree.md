...
| REQ | Status | Evidence |
|---|---|---|
| REQ-1 (criteria — gini/entropy/log_loss + mse/friedman_mse/absolute_error/poisson) | NOT-STARTED | **Compile gap.** The shared `ClassificationCriterion` (`decision_tree.rs`) gained `LogLoss`; extra_tree.rs's private `fn compute_impurity` matches only `{Gini, Entropy}` → `error[E0004]: non-exhaustive patterns: ClassificationCriterion::LogLoss not covered` at `extra_tree.rs:798` — **the crate does not compile** (verified: `cargo build -p ferrolearn-tree` fails). The classifier accepts only Gini/Entropy; the regression finders (`fn find_random_regression_split`) are hard-wired to MSE/variance and silently ignore `FriedmanMse`/`AbsoluteError`/`Poisson` even when set via `with_criterion`. open prereq blockers #680 (LogLoss arm in `compute_impurity` — one-line fix `Entropy \| LogLoss => entropy_impurity`, mirroring `decision_tree.rs:1012`; unblocks the whole crate), #681 (regression criteria friedman_mse/absolute_error/poisson + poisson y-guard). |
