...
- `struct DecisionTreeClassifier<F>` with `max_depth: Option<usize>`,
  ...
  `with_min_impurity_decrease`, `with_ccp_alpha`; `DecisionTreeRegressor<F>`
  analogous. `max_features`, `random_state`, `max_leaf_nodes`, `class_weight`,
  `splitter`, `monotonic_cst` are still absent at the estimator surface.
...
- REQ-3 (stopping / pruning params): `max_depth`, `min_samples_split`,
  ...
  (#662), `ccp_alpha` (cost-complexity pruning, #663) present; `max_leaf_nodes`
  (best-first, #661) still absent, all with sklearn semantics.
