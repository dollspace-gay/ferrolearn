...
- REQ-3 (stopping / pruning params): `max_depth`, `min_samples_split`,
  ...
  (#662), `ccp_alpha` (cost-complexity pruning, #663), `max_leaf_nodes`
  (best-first, #661) present, all with sklearn semantics.
...
| REQ-3 (stopping/pruning params) | NOT-STARTED (partial — #662 + #664 + #663 + #661 closed) | impl: `max_depth`, `min_samples_split`, `min_samples_leaf` are wired (`fn build_classification_tree` stop check `n < params.min_samples_split \|\| depth >= max_depth`) and unit-tested (`test_classifier_max_depth_1`, `test_classifier_min_samples_leaf`). **`min_impurity_decrease` (#662) SHIPPED**: `pub min_impurity_decrease: F` + `#[must_use] with_min_impurity_decrease` on BOTH `DecisionTreeClassifier`/`DecisionTreeRegressor` (default `0.0`, mirroring `_classes.py:946`/`:1317`); the gate lives in `struct ImpurityGate` (threaded through `fn build_classification_tree`/`fn build_regression_tree`), rejecting a split when the tree-normalized improvement `N_t/N·(parent − N_tL/N_t·imp_L − N_tR/N_t·imp_R)` satisfies `improvement + EPSILON < min_impurity_decrease` (`ImpurityGate::rejects`, `_tree.pyx:284`; `EPSILON = F::epsilon()` = `np.finfo('double').eps`, `_tree.pyx:63`). The improvement is recovered from the finder's stored `best_impurity_decrease`: `/N` for gini/entropy/MSE/MAE/poisson, `/N_t` for friedman_mse whose finder score IS `FriedmanMSE.impurity_improvement = diff²/(n_L·n_R·n_t)` (`_criterion.pyx:1573`). The finders' accept gate was relaxed `> 0` → `>= 0` so zero-improvement splits the default `0.0` gate accepts are returned (sklearn's `>=`-semantics). **`min_weight_fraction_leaf` (#664) SHIPPED**: `pub min_weight_fraction_leaf: F` + `#[must_use] with_min_weight_fraction_leaf` on BOTH (default `0.0`); folded ONCE at fit into the effective per-child minimum `max(min_samples_leaf, ceil(min_weight_fraction_leaf · N))` (`fn effective_min_samples_leaf`, uniform-weight `min_weight_leaf = min_weight_fraction_leaf · N` of `_classes.py:371`, child-reject `weighted_n < min_weight_leaf` of `_splitter.pyx:470`), fed as `params.min_samples_leaf` into the existing split-finder child-size gate. **Production consumers** (R-DEFER-1): both params are reachable via the new `with_*` builders on the re-exported `DecisionTreeClassifier`/`DecisionTreeRegressor` (`lib.rs` `pub use`); the gate/fold are invoked from `DecisionTreeClassifier::fit`/`DecisionTreeRegressor::fit`; the forest builders pass `ImpurityGate::disabled` (byte-identical, no `min_impurity_decrease` exposure). **`ccp_alpha` (#663) SHIPPED**: `pub ccp_alpha: F` + `#[must_use] with_ccp_alpha` on BOTH estimators (default `0.0`, `Interval(Real, 0.0, None, closed="left")`, `_classes.py:123`/`:946`/`:1317`); applied at the END of `fit` (after the tree is built) only when `ccp_alpha > 0` (`fn prune_ccp`, the native analog of `_cost_complexity_prune` + `_build_pruned_tree_ccp`, `_tree.pyx:1649,1808`). Each node's resubstitution risk `R(t) = impurity(t)·n_t/N` (sklearn `r_node`, `_tree.pyx:1711`); the internal node with the SMALLEST `effective_alpha = (R(t) − R(T_t))/(n_leaves(T_t) − 1)` is collapsed while `effective_alpha <= ccp_alpha` (sklearn stops when `ccp_alpha < effective_alpha`, `_AlphaPruner.stop_pruning`, `_tree.pyx:1617`), recomputing after each collapse via the weakest-link loop. Per-node `impurity`/`n_samples`/collapse `value`/`distribution` are recorded during the build in a side `Vec<NodeMeta<F>>` (built ONLY when `ccp_alpha > 0`, threaded as `Option<&mut Vec<NodeMeta>>` through `fn build_classification_tree`/`fn build_regression_tree`; forest builders pass `None` ⇒ byte-identical, no allocation); `fn rebuild_pruned_tree` re-emits a compacted depth-first-preorder flat `Vec<Node<F>>` with remapped child indices so the surviving collapsed leaves carry the original node's own value/distribution (matching sklearn copying the node value into the pruned leaf). **Production consumers** (R-DEFER-1): `ccp_alpha` is reachable via `with_ccp_alpha` on the re-exported `DecisionTreeClassifier`/`DecisionTreeRegressor` (`lib.rs` `pub use`); `prune_ccp` is invoked from `DecisionTreeClassifier::fit`/`DecisionTreeRegressor::fit`. **Smoke tests** (R-CHAR-3, live sklearn 1.5.2 oracle): `test_classifier_ccp_alpha_default_node_count_7` (0.0 → 7, predict `[0,0,0,1,1,1,2,2,0]`), `_0_1_node_count_7` (0.1 < weakest-link 0.14815 → 7), `_0_3_node_count_3` (0.3 prunes the 0.14815 subtree → 3, `[0,0,0,1,1,1,0,0,0]`); `test_regressor_ccp_alpha_default_node_count_15` (0.0 → 15, predict == y), `_0_001_node_count_15` (0.001 < weakest-link 0.00208 → 15), `_0_05_node_count_3` (0.05 → 3, mean leaves `[1.05×4, 5.05×4]`). **Smoke tests** (R-CHAR-3, live sklearn 1.5.2 oracle, 9×2 set): `test_classifier_min_impurity_decrease_default_node_count_7` (node_count 7, predict `[0,0,0,1,1,1,2,2,0]`), `_0_2_node_count_3` (mid=0.2 → 3, `[0,0,0,1,1,1,0,0,0]`), `_0_5_node_count_1` (mid=0.5 → 1, all-0), `test_classifier_min_weight_fraction_leaf_0_25_node_count_5` (mwfl=0.25 → 5, `[0,0,0,1,1,1,0,0,0]`), `test_effective_min_samples_leaf_fold`. **`max_leaf_nodes` (#661) SHIPPED**: `pub max_leaf_nodes: Option<usize>` + `#[must_use] with_max_leaf_nodes` on BOTH `DecisionTreeClassifier`/`DecisionTreeRegressor` (default `None`, `Interval(Integral, 2, None, closed="left")`, `_classes.py:121`/`:946`/`:1317`); when `Some(k)`, `fit` dispatches to the best-first builder (`fn build_classification_tree_best_first` / `fn build_regression_tree_best_first`, the native analog of `BestFirstTreeBuilder.build`, `_tree.pyx:407,427`). A frontier (`struct FrontierRecord`) is grown as a max-heap on each node's tree-normalized best-split improvement (`fn pop_best_frontier`, the highest-improvement record popped first, mirroring `_compare_records`/`pop_heap`, `_tree.pyx:392,488`; exact-improvement ties broken by insertion order `seq` — DOCUMENTED as a boundary since sklearn's C++ heap is unstable on exact ties, but the oracle sets have distinct improvements). `max_split_nodes = max_leaf_nodes − 1` (`_tree.pyx:457`) is decremented per materialized split; when it reaches 0 the remaining frontier nodes become leaves, yielding `k` leaves / `2k−1` nodes. The split-finder + `ImpurityGate` are REUSED (a frontier node can split only if it passes max_depth / min_samples_split / min_samples_leaf / effective_min_samples_leaf / min_impurity_decrease / non-pure). The frontier arena is serialized depth-first pre-order (`fn serialize_best_first_arena`) into the same flat `Vec<Node<F>>` layout `predict`/`nodes()` already understand. The regressor heap key uses a numerically-stable two-pass centered variance (`fn stable_regression_improvement` / `fn centered_variance`) so the naive `Σy²/n − mean²` cancellation cannot flip the relative order of two ~2e-17-apart improvements vs sklearn's running-sum criterion (exposed at the k=4 regressor oracle); the finder's stored `impurity_decrease` is UNCHANGED, so feature-importances and the depth-first path stay byte-identical. **Production consumers** (R-DEFER-1): `max_leaf_nodes` is reachable via `with_max_leaf_nodes` on the re-exported `DecisionTreeClassifier`/`DecisionTreeRegressor` (`lib.rs` `pub use`); the best-first builders are invoked from `DecisionTreeClassifier::fit`/`DecisionTreeRegressor::fit`. **Smoke tests** (R-CHAR-3, live sklearn 1.5.2 oracle): `test_classifier_max_leaf_nodes_2`/`_3`/`_4_equals_unlimited`/`_none_unchanged` (9×2 set: k=2 → node_count 3 / 2 leaves / `[0,0,0,1,1,1,0,0,0]`; k=3 → 5 / 3 / `[0,0,0,1,1,1,0,2,0]`; k=4 → 7 / 4 / `[0,0,0,1,1,1,2,2,0]` == unlimited; None → 7 / `[0,0,0,1,1,1,2,2,0]`), `test_regressor_max_leaf_nodes_2`/`_3`/`_4`/`_5`/`_none_unchanged` (8×1 set `yr=[1,1.2,0.9,1.1,5,5.2,4.9,5.1]`: k=2 → 3 / 2 / `[1.05×4,5.05×4]`; k=3 → 5 / 3 / `[1.05×4,5.1,5.1,5.0,5.0]`; k=4 → 7 / 4 / `[1.05×4,5.0,5.2,5.0,5.0]`; k=5 → 9 / 5 / `[1.05×4,5.0,5.2,4.9,5.1]`; None → 15 / == y). The regressor finder retains the `> 0` accept (so the zero-improvement `min_impurity_decrease` boundary for regression remains pre-existing divergence #660/#3); the classifier accept is `>= 0`. STILL NOT-STARTED on this module: `max_features`/`random_state`/`class_weight`/multi-output (open prereq blockers #665/#670/#669/#668). |
...
The default builder is recursive depth-first (`fn build_classification_tree`,
`fn build_regression_tree`), structurally the `DepthFirstTreeBuilder`
(`_tree.pyx`); when `max_leaf_nodes.is_some()`, `fit` instead dispatches to the
best-first builders (`fn build_classification_tree_best_first` /
`fn build_regression_tree_best_first`), the native `BestFirstTreeBuilder`
(`_tree.pyx:407`). Both paths share the split-finder, the `ImpurityGate`
(`min_impurity_decrease`), and the `ccp_alpha` post-pruning. The split-finder
...
- **#661** — Blocker for REQ-3 of decision_tree: add `max_leaf_nodes` (best-first
  builder) — extends existing issue #51. **CLOSED** — `pub max_leaf_nodes:
  Option<usize>` + `with_max_leaf_nodes` on both estimators (default `None`);
  `fn build_classification_tree_best_first`/`fn build_regression_tree_best_first`
  grow a max-heap frontier on tree-normalized improvement (`fn pop_best_frontier`,
  sklearn `_compare_records`/`pop_heap`, `_tree.pyx:392,488`), `max_split_nodes =
  k − 1` (`_tree.pyx:457`), reusing the split-finder + `ImpurityGate`; the arena
  is serialized depth-first pre-order (`fn serialize_best_first_arena`). The
  regressor heap key uses two-pass centered variance (`fn
  stable_regression_improvement`) to match sklearn's frontier order on near-ties.
...
- **#663** — Blocker for REQ-3 of decision_tree: add `ccp_alpha` cost-complexity
  pruning — extends existing issue #49. **CLOSED** — `pub ccp_alpha` +
  `with_ccp_alpha` on both estimators (default `0.0`); `fn prune_ccp` runs the
  Breiman weakest-link prune at the end of `fit` (`R(t)=impurity·n_t/N`,
  collapse while `effective_alpha <= ccp_alpha`, sklearn
  `_cost_complexity_prune`/`_AlphaPruner.stop_pruning`, `_tree.pyx:1649,1617`);
  per-node impurity/value recorded in a side `Vec<NodeMeta>` built only when
  `ccp_alpha > 0`; `fn rebuild_pruned_tree` re-emits the compacted flat vec.
