...
- REQ-4: **Classifier aggregation = SOFT vote.** `predict` must be `argmax` of
  the *mean of per-tree `predict_proba`* (sklearn `:904-907`), NOT a hard
  per-tree-label majority. ferrolearn currently hard-votes.
...
| REQ-4 (classifier SOFT vote) | NOT-STARTED | open prereq blocker #675. `predict` (`random_forest.rs`) tallies per-tree predicted labels (`votes[class_idx] += 1`) and returns `argmax` of integer vote counts (HARD majority); sklearn `predict` (`_forest.py:904-907`) = `argmax` of mean `predict_proba` (SOFT). Live oracle: on 200 `max_depth=2` samples `predict==soft` (True), `predict==hard` (False), 30/200 rows differ — a real divergence, not RNG. Fix: route `predict` through `predict_proba`. |
