...
| REQ-API-ACCESSORS (`.shape`/`.data`/`.row`/`.col`) | NOT-STARTED | blocker issue to be filed by critic. scipy exposes `.shape` (tuple, `_coo.py:32` ctor) and `.data`/`.row`/`.col` (`_coo.py:106`/`:122`) as first-class accessors; ferrolearn exposes `n_rows()`/`n_cols()` (NO `.shape()` tuple) and hides the triplet arrays behind `inner()`/`into_inner()` (the only way to reach the row/col/data slices is `coo.inner().row_inds()/.col_inds()/.data()` on the `sprs::TriMat`). No public `shape`/`row`/`col`/`data` accessor on `CooMatrix` (`grep -n "pub fn" coo.rs`). R-DEV-2/3 attribute-contract divergence. |
| REQ-MISSING-METHODS (rest of coo_matrix surface) | NOT-STARTED | blocker issue to be filed by critic. `coo.rs` has only `new`/`with_capacity`/`from_triplets`/`push`/`n_rows`/`n_cols`/`nnz`/`inner`/`into_inner`/`to_dense`. Absent vs scipy `coo_matrix`: `tocsr`/`tocsc`/`todense` (`_coo.py:349`/`:316`; conversion EXISTS but as `CsrMatrix::from_coo`/`CscMatrix::from_coo` in sibling modules, NOT as a `.tocsr()` method on `CooMatrix`), `transpose`/`T` (`_coo.py:229`), `sum`/`sum(axis=)` (`_coo.py:1429`), `sum_duplicates` (`_coo.py:768`), `eliminate_zeros` (`_coo.py:798`), `diagonal` (`_coo.py:458`), `multiply`, `dot`/`@` (`_coo.py:1045`), arithmetic `+`/`-`/`*` (`_coo.py:811`/`:835`/`:847`), `power`, `max`/`min`, `astype`, `copy`, `todia`/`todok`. Live oracle: `m.T.toarray()`=`[[3,0],[0,0],[0,5]]`, `m.sum()`=8.0, `m.sum(axis=0)`=`[[3,0,5]]`, `m.diagonal()`=`[3,0]`, `m.sum_duplicates()` collapses nnz 3→2 with `data=[3,5]`. |
