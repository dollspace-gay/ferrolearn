...
| REQ-MISSING-METHODS (rest of coo_matrix surface) | NOT-STARTED | open prereq blocker #1997. Still absent: `sum`/`sum(axis=)` (`_coo.py:1429`), `sum_duplicates` (`:768`), `eliminate_zeros` (`:798`), `diagonal` (`:458`), `multiply`, `dot`/`@` (`:1045`), arithmetic `+`/`-`/`*` (`:811`/`:835`/`:847`), `power`, `max`/`min`, `astype`, `copy`, `todia`/`todok`. Live oracle: `m.sum()`=8.0, `m.sum(axis=0)`=`[[3,0,5]]`, `m.diagonal()`=`[3,0]`, `m.sum_duplicates()` collapses nnz 3→2 with `data=[3,5]`. |
