...
| REQ-4 (OAS) | NOT-STARTED | open prereq blocker (tracking #1701; critic to file specifics). `pub fn fit for OAS in covariance.rs` uses the SUPERSEDED OAS formula `rho_num = (1 - 2/p)*tr(S²) + tr(S)²`, `rho_den = (n + 1 - 2/p)*(tr(S²) - tr(S)²/p)` — but sklearn 1.5.2 uses `alpha = mean(emp_cov²)`, `num = alpha + mu²`, `den = (n_samples+1)*(alpha - mu²/p)` (`_shrunk_covariance.py:79-87`). Live oracle (asymmetric probe): sklearn `shrinkage_=0.6705854984555868`, `covariance_[0][0]=5.627443884884855`; ferrolearn `shrinkage=0.5386971696301457`, `covariance[0][0]=5.2855111805226` — DIVERGES. Oracle-pinnable as a failing test. |
...
| REQ-11 (contamination domain) | NOT-STARTED | open prereq blocker (tracking #1701). `pub fn fit for EllipticEnvelope in covariance.rs` rejects `contamination >= 0.5` (`self.contamination <= 0.0 \|\| self.contamination >= 0.5`), but sklearn's constraint is `Interval(Real, 0, 0.5, closed="right")` (`_elliptic_envelope.py:147`) — `0.5` is VALID in sklearn. ferrolearn over-rejects the right endpoint. |
...
| REQ-13 (mahalanobis squared) | NOT-STARTED | open prereq blocker (tracking #1701). `pub fn mahalanobis in covariance.rs` (`FittedCovariance::mahalanobis` → `mahalanobis_distances`) returns `val.abs().sqrt()` — the NON-squared distance. sklearn's `EmpiricalCovariance.mahalanobis` returns the SQUARED distance (`_empirical_covariance.py:340` docstring "Squared Mahalanobis distances", `:353` `dist = pairwise_distances(...)**2` semantics). Live oracle `X=[[0,0],[2,0],[0,2],[2,2]]`: sklearn `mahalanobis = [2.0, 2.0, 2.0, 2.0]` (squared); ferrolearn returns `sqrt(2)≈1.414` each. Divergence by a square. Oracle-pinnable. |
| REQ-14 (score/log_likelihood) | NOT-STARTED | open prereq blocker (tracking #1701). No `score` method exists on any covariance estimator struct. `pub fn log_likelihood in helpers.rs` computes the Gaussian log-likelihood but is not wired into a `score(X)` method mirroring `EmpiricalCovariance.score` (`_empirical_covariance.py:258`, `log_likelihood(test_cov, self.get_precision())`). Live oracle (symmetric probe): sklearn `score(X) = -2.8378770664093453`. No `score` accessor → NOT-STARTED. Oracle-pinnable once a `score` method exists. |
| REQ-15 (error_norm) | NOT-STARTED | open prereq blocker (tracking #1701). No `error_norm` method exists on any covariance struct; sklearn exposes `error_norm(comp_cov, norm='frobenius', scaling=True, squared=True)` (`_empirical_covariance.py:289`). Absent. |
...
| REQ-17 (collapsible_if lints) | NOT-STARTED | open prereq blocker (tracking #1701). `cargo clippy -p ferrolearn-covariance --all-targets -- -D warnings` FAILS with two `clippy::collapsible_if` errors at `covariance.rs:1186` (`if let Ok(ld) = log_det_spd(&cov) { if ld < best_log_det {...} }`) and `covariance.rs:1254` (`if well_conditioned { if let Ok(inv) = spd_inverse(m) {...} }`). These block the crate gauntlet (R-DEFER-6). Fix: let-chain collapse (MSRV 1.88 supports `if let ... && ...`). Mechanically checkable. |
