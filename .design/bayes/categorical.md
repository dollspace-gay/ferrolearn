...
| REQ-10a (Rust fitted accessors) | SHIPPED | `FittedCategoricalNB<F>` gains `#[must_use]` getters `feature_log_prob() -> &[Vec<Vec<F>>]` (`&feature_log_prob`, `[feature][class][category]`, sklearn `feature_log_prob_` `:1273`), `category_count() -> &[Vec<Vec<usize>>]` (`&category_counts`, `:1266`), `n_categories() -> Vec<usize>` (`categories[i].len()` per feature, `:1303`), `class_count() -> Array1<F>` (`class_counts` cast to `F`), `class_log_prior() -> &[F]` (`&class_log_prior`, `:600`). Per-feature `categories` are sorted ascending (`cats.sort_unstable(); cats.dedup()` in fit) so category index == value for contiguous fixtures. Verification (live sklearn 1.5.2, R-CHAR-3, `X=[[0,1],[1,0],[0,0],[2,1],[2,0],[1,1]]`, `y=[0,0,0,1,1,1]`): `n_categories_=[3,2]`, `class_count_=[3,3]`, `class_log_prior_=[-0.6931471806,-0.6931471806]`, `category_count_[0]=[[2,1,0],[0,1,2]]`/`[1]=[[2,1],[1,2]]`, `feature_log_prob_[0]=[[-0.69314718,-1.09861229,-1.79175947],[-1.79175947,-1.09861229,-0.69314718]]`/`[1]=[[-0.51082562,-0.91629073],[-0.91629073,-0.51082562]]`. Tests `categorical_n_categories_class_count_prior_match_sklearn`, `categorical_feature_log_prob_and_category_count_match_sklearn`. |
| REQ-10b (PyO3 binding + sample_weight) | NOT-STARTED | open prereq blocker **#923** (multi-file `ferrolearn-python`). **CategoricalNB has NO PyO3 binding** — unlike `_RsMultinomialNB`/`_RsBernoulliNB`/`_RsComplementNB` (`extras.rs`) + `ferrolearn.MultinomialNB`/`BernoulliNB`/`ComplementNB` (`_extras.py`), there is NO `_RsCategoricalNB` and NO `ferrolearn.CategoricalNB`, so `import ferrolearn` cannot reach CategoricalNB at all (the Rust accessors now exist — REQ-10a — but are not surfaced to Python). Also subsumes the `class_prior` wrong-length error-TYPE sub-item (REQ-2) and the negative-feature MESSAGE/TYPE sub-item (REQ-7). |
