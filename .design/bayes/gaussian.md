...
| REQ-9 (fitted attrs `theta_`/`var_`/`epsilon_`/`class_count_`/`class_prior_`) | NOT-STARTED | open prereq blocker **#895**. sklearn exposes `theta_`, `var_`, `epsilon_`, `class_count_`, `class_prior_`, `classes_` (`:171-202`). `FittedGaussianNB` stores `theta`/`sigma`/`raw_sigma`/`class_counts`/`log_prior`/`var_smoothing` as PRIVATE fields with no accessor; only `classes()` (via `HasClasses`) is public. No `epsilon_`/`class_prior_` getter. Pin (AC-8): `hasattr(sklearn GaussianNB fitted, 'theta_')` True; ferrolearn fitted type exposes only `classes()`. |
