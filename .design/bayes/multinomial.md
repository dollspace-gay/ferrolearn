...
| REQ-9a (Rust fitted accessors) | SHIPPED | `FittedMultinomialNB<F>` gains `#[must_use]` getters `feature_log_prob()` (`&log_theta`, sklearn `feature_log_prob_` `:892`), `class_log_prior()` (`&log_prior`, `:600`), `feature_count()` (`&feature_counts`, `:880`), `class_count()` (`class_counts` cast to `F`, `:879`). `coef_`/`intercept_` are intentionally NOT exposed — DEPRECATED/REMOVED in sklearn 1.5.2 (`MultinomialNB().coef_` raises `AttributeError`). Verification (live sklearn 1.5.2, R-CHAR-3, `X=[[3,1,0],[2,0,1],[4,2,0],[0,1,4],[1,0,3],[0,2,5]]`, `y=[0,0,0,1,1,1]`): `feature_log_prob_=[[-0.4700036292,-1.3862943611,-2.0794415417],[-2.2512917986,-1.558144618,-0.3794896217]]`, `class_log_prior_=[-0.6931471806,-0.6931471806]`, `feature_count_=[[9,3,1],[1,3,12]]`, `class_count_=[3,3]`. Tests `multinomial_feature_log_prob_and_class_log_prior_match_sklearn`, `multinomial_feature_count_and_class_count_match_sklearn`. |
| REQ-9b (PyO3 surface) | NOT-STARTED | open prereq blocker **#902** (multi-file `ferrolearn-python`). `_RsMultinomialNB` (`extras.rs`, the `py_classifier!` macro) exposes ONLY `new(alpha=1.0, fit_prior=true)` + `fit` + `predict` — NO `class_prior`/`force_alpha` kwargs, NO `predict_proba`/`predict_log_proba`/`predict_joint_log_proba`/`score`/`partial_fit` (which the library HAS), NO fitted-attr getters (the Rust accessors now exist — REQ-9a — but are not surfaced to Python). Also subsumes the negative-feature MESSAGE-parity sub-item (REQ-5). |
