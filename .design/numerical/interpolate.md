...
| REQ-DEFAULT-BC (default bc_type) | NOT-STARTED | open prereq blocker (to be filed by critic). Live oracle: `inspect.signature(CubicSpline.__init__).parameters['bc_type'].default` = `'not-a-knot'`. ferrolearn `pub fn new(x, y, bc: BoundaryCondition)` makes `bc` a REQUIRED positional argument with no default — a caller porting scipy's `CubicSpline(x, y)` must explicitly pass `BoundaryCondition::NotAKnot` or get a compile error. R-DEV-2 (constructor defaults are part of the user-API ABI). Fix: a builder/`Default`-bearing constructor (or a `new_default`/`with_bc`) so the no-BC path resolves to not-a-knot, matching scipy. |
...
| REQ-ERR-TYPE (FerroError + ValueError) | NOT-STARTED | open prereq blocker (to be filed by critic). `pub fn new` returns `Result<Self, String>` (`Err(format!("x must be strictly increasing, …"))`), not `Result<_, FerroError>` (CLAUDE.md error-handling, R-CODE-2). scipy raises `ValueError` on unsorted/duplicate `x` (verified: `CubicSpline([0,2,1],[0,1,2])` → `ValueError`). Substrate convention divergence. |
