...
| REQ-7 (error type — FerroError) | SHIPPED (#1975) | FIXED — `gauss_legendre`/`gauss_legendre_composite`/`gl_nodes_weights` now return `Result<_, FerroError>`; the two `Err` sites map to `FerroError::InvalidParameter { name, reason }` (messages preserved: `"Gauss-Legendre order must be in 1..=20, got {n}"`, `"n_panels must be at least 1"`), mirroring scipy's `ValueError` on invalid integration parameters (CLAUDE.md / R-CODE-2). `ferrolearn-core` is a workspace dep (added #1961). Guards `tests/divergence_integrate.rs::gauss_legendre_invalid_order_returns_ferroerror` (orders 0 and 21) + `::gauss_legendre_composite_invalid_npanels_returns_ferroerror` (`matches!` on `Err(FerroError::InvalidParameter)`). REMAINDER (separate sub-concern, not a String-error issue): `quad`/`quad_with_limit` still return a bare `QuadratureResult` with no error channel — non-convergence is reported as value + `error_estimate` (matches scipy `quad`, which also does not raise on slow convergence). |
