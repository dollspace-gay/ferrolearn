...
| REQ-10 (error type / convergence signalling) | NOT-STARTED | open prereq blocker (to be filed by critic). `NewtonCG`/`TrustRegionNCG`/`Powell::minimize` return `Result<OptimizeResult, String>` — the only `Err` is `"initial guess x0 must have at least one element"`; non-convergence is an `Ok` with `converged: false` (matches scipy's no-raise behaviour, but via a bool flag not `status`/`message`). `brent_bounded` returns a BARE `Minimize1DResult` (NO `Result` at all; non-convergence is `success: false`). Neither uses `ferrolearn_core::error::FerroError` (CLAUDE.md: "All public functions return `Result<T, FerroError>`"; R-CODE-2), and the `String`/bool signalling does not map to scipy's `OptimizeResult.success`/`status`/`message` for the `ferrolearn-python` boundary (R-DEV-2/3). The crate-wide error contract is not satisfied. |
