...
| REQ-9 (error type — FerroError) | SHIPPED (#1983) | FIXED — `eigsh`/`solve`/`solve_sparse`/`solve_impl` now return `Result<EigenResult, FerroError>`. The 4 validation `Err`s (non-square `mat`, `k` < 1, `k` > n, `ncv` < k+1) map to `FerroError::InvalidParameter { name, reason }` (scipy `ValueError`); the non-convergence `Err` ("Lanczos solver did not converge within {} iterations") maps to `FerroError::NumericalInstability { message }` (scipy `ArpackNoConvergence`). Messages preserved; `ferrolearn-core` is a workspace dep (#1961). Guard `tests/divergence_sparse_eig.rs::sparse_eig_invalid_params_return_ferroerror` (`matches!` on `Err(FerroError::InvalidParameter)` for non-square + k=0). The error TYPE only — the `k == n` acceptance behavior (scipy raises `TypeError`) remains the separate REQ-8 / #1982. |
