...
| REQ-11 (FerroError error type) | NOT-STARTED | open prereq blocker (to be filed by critic). Every constructor returns `Result<Self, String>` (`Normal::new`/`ChiSquared::new`/`FDist::new`/`StudentsT::new`/`Beta::new`/`Gamma::new`/`Dirichlet::new` all `.map_err(|e| format!(...))`). The workspace contract (R-DEV-2 / CLAUDE.md "All public functions return `Result<T, FerroError>`") requires `ferrolearn_core::error::FerroError`, not an ad-hoc `String`. Blocked-with REQ-12 (the ferray-substrate migration is the natural point to swap the error type). |
