...
| REQ-10 (no-panic on invalid params) | SHIPPED (#1970) | FIXED — no `panic!`/`assert!`/`.expect` on user-reachable input. `unwrap_stat` now maps an undefined moment (statrs `None`, e.g. StudentsT df ≤ 2 variance) to `f64::NAN` (was `panic!`); `Dirichlet::ln_pdf` returns `f64::NAN` for invalid `x` (length mismatch / element outside `(0,1)` / sum ≠ ≈1 at 1e-4 tol, was `assert!`); `Dirichlet::sample` replaced `rand_distr::Gamma::new(..).expect(..)` with `match { Ok => sample, Err(_) => 0.0 }` (Err unreachable — alpha validated `> 0` at construction). Mirrors scipy.stats (undefined moments → `nan`/`inf`; `dirichlet.logpdf` on bad `x` → `-inf`/raises, never an unhandled crash) — CLAUDE.md / R-CODE-2 / R-APG-1. Earlier `chi2_sf`/`f_sf`/`t_test_two_tailed` nan path was FIXED #1966. Guards `students_t_undefined_variance_is_nan_not_panic` + `dirichlet_ln_pdf_invalid_x_is_nan_not_panic`. FINER REMAINDER (separate nicety, not a panic): exact undefined-moment value parity (our `nan` vs scipy's occasional `inf`, e.g. `t(df=2).var()`) is not pinned. |
