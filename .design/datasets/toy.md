...
| REQ-3 (wine parity) | SHIPPED | impl `pub fn load_wine in toy.rs` → `parse_classification_csv(WINE_CSV, 178, 13)`, where `const WINE_CSV` embeds `data/wine_data.csv` (copied verbatim from `sklearn/datasets/data/wine_data.csv`) via `include_str!`. Mirrors `sklearn/datasets/_base.py:496` (`load_wine`). Consumer: re-exported as a boundary pub API by `pub use toy::load_wine` in `ferrolearn-datasets/src/lib.rs` and exercised by the production-surface proof `tests/api_proof.rs::api_proof_toy_loaders` (test-only API proof). Oracle parity: live `load_wine(return_X_y=True)[0][0] == [14.23,...,1065.0]`, `y[0]==0` — matched element-wise (`<1e-6`). Pinned by `tests/divergence_toy.rs::divergence_wine_x0`. Verification: `cargo test -p ferrolearn-datasets` → 92+5+6+6+15 passed, 0 failed. |
| REQ-4 (breast_cancer parity) | SHIPPED | impl `pub fn load_breast_cancer in toy.rs` → `parse_classification_csv(BREAST_CANCER_CSV, 569, 30)`, where `const BREAST_CANCER_CSV` embeds `data/breast_cancer.csv` (copied verbatim) via `include_str!`. Mirrors `sklearn/datasets/_base.py:750` (`load_breast_cancer`). Consumer: `pub use toy::load_breast_cancer` in `lib.rs` + `tests/api_proof.rs::api_proof_toy_loaders`. Oracle parity: live `X[0,:3]==[17.99,10.38,122.8]`, `y[0]==0` (malignant) — matched element-wise (`<1e-6`). Pinned by `tests/divergence_toy.rs::divergence_breast_cancer_x0`. Verification: same `cargo test` run. |
| REQ-5 (diabetes parity, scaled default) | SHIPPED | impl `pub fn load_diabetes in toy.rs` → `parse_whitespace_matrix(DIABETES_DATA_CSV, 442, 10)` then `scale_diabetes_columns(&mut x)` then `parse_float_column(DIABETES_TARGET_CSV, 442)`. `const DIABETES_DATA_CSV`/`DIABETES_TARGET_CSV` embed the gunzipped `data/diabetes_data_raw.csv` (whitespace-delimited) + `data/diabetes_target.csv`. `scale_diabetes_columns` mirrors `sklearn/datasets/_base.py:1132-1134`: per column `scaled[i,j] = (raw[i,j] - mean_j) / pop_std_j / sqrt(442)` (population std, ddof=0; stats accumulated in f64). Mirrors `_base.py:1044` (`load_diabetes`, default `scaled=True`). Consumer: `pub use toy::load_diabetes` in `lib.rs` + `tests/api_proof.rs::api_proof_toy_loaders`. Oracle parity: live `X[0,:3]==[0.038075906433423026,0.05068011873981862,0.061696206518683294]`, `y[:3]==[151.0,75.0,141.0]` — matched element-wise (`<1e-9`); the manual scaling reproduction verified `np.allclose(manual, oracle)==True`. Pinned by `tests/divergence_toy.rs::divergence_diabetes_x0_y`. Verification: same `cargo test` run. |
| REQ-6 (digits full + n_class) | PARTIAL | impl `pub fn load_digits in toy.rs` → `parse_headerless_classification_csv(DIGITS_CSV, 1797, 64)`, where `const DIGITS_CSV` embeds the gunzipped `data/digits.csv` (comma-delimited, 65 cols = 64 pixels + label) via `include_str!`. Mirrors `sklearn/datasets/_base.py:907` (`load_digits`, default `n_class=10`). Consumer: `pub use toy::load_digits` in `lib.rs` + `tests/api_proof.rs::api_proof_toy_loaders`. Oracle parity (full default): live `load_digits(return_X_y=True)` shape `(1797,64)`, `X[0,:8]==[0,0,5,13,9,1,0,0]` — matched element-wise (`<1e-9`). Pinned by `tests/divergence_toy.rs::divergence_digits_shape_and_x0`. REMAINING GAP: the upstream `n_class` parameter (trim to first `n_class` digit classes) is not exposed — the no-arg loader returns the full `n_class=10` default. Verification: same `cargo test` run. |
