...
| REQ-11 (fitted attributes) | NOT-STARTED | tracking #1661. `FittedKernelRidge` exposes `dual_coef()` and `x_fit()` accessors, but sklearn's attribute names are `dual_coef_`, `X_fit_`, and `n_features_in_` (`kernel_ridge.py:85-94`). No `n_features_in_` accessor exists; the Python binding does not surface `X_fit_`/`n_features_in_`. Naming/coverage gap. |
