...
| REQ-9 (ShapeMismatchContext builder) | SHIPPED | impl `struct ShapeMismatchContext` + `new`/`expected`/`actual`/`build in error.rs`. Non-test production consumer: `fn check_consistent_length in dataset.rs` builds its `FerroError::ShapeMismatch` via `ShapeMismatchContext::new(..).expected(..).actual(..).build()` (REQ-4 validation surface), and `check_consistent_length` is consumed by `Fit::fit for Pipeline in pipeline.rs`. Behavior-preserving: `.build()` yields the same `ShapeMismatch` variant as the prior direct construction (AC-9). |
