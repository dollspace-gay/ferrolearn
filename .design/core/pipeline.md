...
| REQ-3 (fit_transform / transform / predict_proba / decision_function / score) | NOT-STARTED | open prereq blocker #361. `FittedPipeline` exposes only `predict`; sklearn's `fit_transform` (`pipeline.py:489`), transformer-final `transform` (`:863`), `predict_proba` (`:675`), `decision_function` (`:731`), `score` (`:961`) have no ferrolearn analog. |
