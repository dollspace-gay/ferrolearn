...
`multi_class` (`ovr`/`crammer_singer`, with the joint Crammer-Singer solver
`fn solve_crammer_singer`, `Solver_MCSVM_CS`, `linear.cpp:493-787`, solver
type 4) is now shipped (REQ-6). Still absent: the unit is on the `ndarray`
substrate, not ferray (REQ-12).
...
`C·w[k]`/base `C`) into the per-sample-`C` solvers. `multi_class` (`Ovr`/`CrammerSinger`,
default `Ovr`) is now a field + `fn with_multi_class in linear_svc.rs`; when
`CrammerSinger`, `fn fit` short-circuits (ignoring penalty/loss/dual,
`_base.py:1017`) to `fn fit_crammer_singer in linear_svc.rs`, which runs the
joint `fn solve_crammer_singer` and applies the binary collapse
(`_classes.py:340-344`). There is **no** `random_state` or `verbose`
field/accessor.
...
(`linear.cpp:2543-2571`). REQ-6 (`multi_class`) is now SHIPPED:
`fn solve_crammer_singer in linear_svc.rs` transcribes liblinear's
`Solver_MCSVM_CS` (`linear.cpp:493-787`, solver type 4, selected
REGARDLESS of penalty/loss/dual per `_base.py:1017,1020-1021`) — flattened
`w[feature*nr_class + m]`, `alpha[i*nr_class + m]` (`Σ_m alpha=0`),
`C[i] = weighted_C[y_i]`, per-sample shrinking (`active_size_i`/`alpha_index`/
`be_shrunk`), the simplex inner solve `fn cs_solve_sub_problem`
(`linear.cpp:541-564`), and the two-level `eps_shrink` stopping
(`linear.cpp:738-753`) — and `fn fit_crammer_singer in linear_svc.rs` extracts
per-class `coef_`/`intercept_` (`_base.py:1240-1245`) and applies the binary
collapse `coef_ = row_1 − row_0`, `intercept_ = int_1 − int_0`
(`_classes.py:340-344`). The module still has no `random_state`/`verbose`,
...
| REQ-6 (multi_class {ovr, crammer_singer}) | SHIPPED | `LinearSVC<F>` exposes `pub multi_class: MultiClass` (`Ovr`/`CrammerSinger`, default `Ovr`, `_classes.py:239`) + `fn with_multi_class in linear_svc.rs`. `multi_class=CrammerSinger` selects liblinear solver type 4 REGARDLESS of penalty/loss/dual (`_get_liblinear_solver_type`, `_base.py:1017,1020-1021`), so `fn fit` short-circuits to `fn fit_crammer_singer in linear_svc.rs` (ignoring penalty/loss/dual) which runs ONE joint solve `fn solve_crammer_singer in linear_svc.rs` — a faithful transcription of `Solver_MCSVM_CS` (`linear.cpp:510`, class body `:493-787`): flattened `w[feature*nr_class + m]`, dual `alpha[i*nr_class + m]` with `Σ_m alpha=0` / `alpha[i,m]<=C[i] if y_i==m else <=0`, `C[i] = weighted_C[y_i]` (`linear.cpp:521-522`; `weighted_C[c] = C·class_weight[c]`), `QD[i]=‖x_i‖²` over the augmented row, `G[m]`/`minG`/`maxG` (`linear.cpp:641-666`), per-sample shrinking via `active_size_i`/`alpha_index`/`fn cs_be_shrunk` (`linear.cpp:566-697`), the simplex inner solve `fn cs_solve_sub_problem` (clone `B→D`, `D[yi]+=Ai·C_yi`, sort DESC, breakpoint `beta`, project, `linear.cpp:541-564`), and the two-level `eps_shrink = max(10·tol,1)` stopping (`linear.cpp:738-753`). Extraction: `coef_[m][feature] = w[feature*nr_class + m]`, `intercept_[m] = intercept_scaling·w[n_features*nr_class + m]` (the augmented bias column, `_base.py:1240-1245`). BINARY (`nr_class==2`): collapse to a single weight vector `coef_ = row_1 − row_0`, `intercept_ = int_1 − int_0`, `is_binary=true` (`_classes.py:340-344`). ferrolearn sweeps natural order (no `bounded_rand_int` shuffle, `linear.cpp:629`); the CS optimum is unique so the limit is identical (documented RNG/shrink-path boundary, as the dual/l1 solvers; `n_iter` is the outer-iteration count, not pinned exactly). Pinned by `test_crammer_singer_smoke in linear_svc.rs` (live oracle: 3-class set `coef [[-0.06762,-0.24341],[0.30048,0.02171],[-0.23286,0.22171]]`, `int [0.91078,-0.62206,-0.28873]`, predict all-correct; binary 8×2 collapse `coef [[0.15504,0.15504]]` shape `(1,2)`, `int [-1.48062]`; ferrolearn within 1e-2). The rigorous oracle pin in `tests/divergence_linear_svc_fit.rs` is the critic's next step. Consumer: `pub use linear_svc::{…} in lib.rs` + `RsLinearSVC` PyO3 binding. |
...
- **#623** — REQ-6 of linear_svc: ~~add the `multi_class` parameter
  (`ovr` default / `crammer_singer`), the `crammer_singer` joint solver~~ —
  SHIPPED (`fn solve_crammer_singer`/`fn fit_crammer_singer`,
  `Solver_MCSVM_CS`, `linear.cpp:493-787`, solver type 4, with the binary
  collapse `_classes.py:340-344`); the rigorous per-class oracle pin in
  `tests/divergence_linear_svc_fit.rs` is the critic's next step.
