...
| REQ-11 (libsvm `Solver_NU` infra for nu_svm.rs) | SHIPPED | `fn solver_nu_core in svm.rs` transcribes libsvm's `Solver_NU` — the nu-specific second-order working-set selection separately over the `y=+1`/`y=-1` groups (the four running maxima `Gmaxp/Gmaxp2/Gmaxn/Gmaxn2`, `svm.cpp:1186-1296`), the shared `Solver::Solve` gradient init + analytic 2-variable box update (`svm.cpp:663-940`), and the two-group `calculate_rho` returning `rho=(r1-r2)/2` and `r=(r1+r2)/2` (`svm.cpp:1370-1418`). Natural-order, no-shrinking, deterministic (like `smo_binary`/`smo_svr`; shrinking is optimum-invariant, R-DEV-7). `pub(crate) fn solve_nu_svc` (init+`/r` rescale, `svm.cpp:1646-1708`) and `pub(crate) fn solve_nu_svr` (`2l`-variable learned-tube, `svm.cpp:1795-1839`) wrap it; `pub(crate) fn FittedSVC::from_nu_ovo` + `pub(crate) struct NuOvoPair` assemble a nu-SVC `FittedSVC` reusing every SVC accessor. Non-test consumer: `NuSVC::fit`/`NuSVR::fit in nu_svm.rs`. Verified by `nu_svm.rs`'s `test_nusvc_oracle_attrs`/`test_nusvr_oracle_attrs` + the now-GREEN `tests/divergence_nu_svm.rs` pins (`.design/linear/nu_svm.md` REQ-2/REQ-3). |
...
- **#641** — REQ-8 of svm: add the sklearn estimator-level parameter surface
  ...
  **CLOSED** — REQ-8 SHIPPED: `shrinking`/`break_ties`/`decision_function_shape`/
  `gamma`/default-alignment landed earlier; `class_weight` is the last part
  (`ClassWeight<F>` + `compute_class_weight` + the `(cp,cn)` `smo_binary`
  generalization + the per-pair `weighted_C` fit wiring, `_base.py:740`).
  `kernel`/`degree`/`coef0` are the documented R-DEV-7 type-parameter design
  difference; `random_state` is deterministic-SMO unused.
