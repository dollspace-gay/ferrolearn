...
| REQ-8 (constructor params/defaults) | NOT-STARTED | open prereq blocker #641. `SVC<F,K>`/`SVR<F,K>` expose only `c`/`tol`/`max_iter`/`cache_size` (+ `epsilon` for SVR), with the kernel (and its degree/coef0/gamma) as the type parameter `K`. **Missing** the sklearn estimator-level surface (R-DEV-2): `kernel` (string), `degree`, `gamma`, `coef0`, `shrinking`, `class_weight`, `decision_function_shape`, `break_ties`, `random_state`; defaults also diverge (`max_iter=10000` vs sklearn `-1` = no limit; `cache_size=1024` vs `200`; no `gamma='scale'` default). |
