...
| REQ | Status | Evidence |
|---|---|---|
| REQ-1 (ONE_CLASS nu dual + nu validation) | NOT-STARTED | open prereq blocker #646. `fn fit in one_class_svm.rs` validates `nu ∈ (0,1]` (`InvalidParameter`) and solves the normalized one-class dual `0≤α≤1/(n·ν), Σα=1` with an SMO-style max/min-gradient working set + analytic update — it *appears* directionally correct, but (a) no `divergence_one_class_svm.rs` pins `dual_coef_`/`intercept_`/`support_` against the live `OneClassSVM(kernel='linear',nu=0.5)` oracle (R-CHAR-1), and (b) the stored `dual_coefs`/`rho` are in the **normalized `Σα=1` scale**, not libsvm's `Σα=ν·n` — so even at the right optimum the exposed values diverge by `ν·n` (oracle `dual_coef_ [[1,0.5,1,1]]`, `Σ=3.5=0.5·7`; ferrolearn would store `[[1,0.5,1,1]]/3.5`). Critic: start here — verify the rescale and pin the linear oracle (`support_ [0,1,2,3]`, `n_support_ [4]`, `intercept_ [-0.01]`). |
| REQ-2 (kernels & gamma resolution) | NOT-STARTED | open prereq blocker #647. The kernel formulas are reused from `crate::svm::Kernel` (correct, per `svm.md` REQ-1), and the three-way `Gamma` enum + `resolved_for_fit` exist in `svm.rs`. **But `fn fit in one_class_svm.rs` never calls `self.kernel.resolved_for_fit(x)`** — it uses the kernel as-constructed, so `gamma='scale'`/`'auto'` are NOT resolved against `X` here and a default rbf one-class SVM uses `gamma=1.0`. No oracle pins `_gamma` or the rbf-`auto` docstring example (`predict [-1,1,1,1,-1]`). |
| REQ-3 (fitted attributes + offset_) | NOT-STARTED | open prereq blocker #648. `FittedOneClassSVM` stores private `support_vectors`/`dual_coefs`/`rho` with **no public `support_`/`support_vectors_`/`n_support_`/`dual_coef_`/`intercept_`/`offset_`/`coef_` accessors** and **no `score_samples` method**. sklearn exposes `dual_coef_` shape `(1,n_SV)`, `n_support_` shape `(1,)`, `intercept_=-rho`, `offset_=-intercept_` (`_classes.py:1767`), linear `coef_=dual_coef_@support_vectors_`. SV indices are not retained (only the SV rows are kept), so `support_` cannot be built without recording the original training-row index. No oracle pins the attribute layout (target: `support_ [0,1,2,3]`, `dual_coef_ [[1,0.5,1,1]]`, `intercept_ [-0.01]`, `offset_ 0.01`, `coef_ [[0.05,0.05]]`). |
| REQ-4 (decision_function / score_samples) | NOT-STARTED | open prereq blocker #649. `pub fn decision_function in one_class_svm.rs` returns `Array1<F>` `(n,)` = `Σ coef·K(sv,x) − rho` — structurally matching sklearn's `(n,)`/`Σ α K − rho` (no sign flip; one-class is excluded from the `_base.py:538` binary flip). **But** the values are in the normalized scale (see REQ-1's `ν·n` divergence), there is **no `score_samples`** method (`= decision_function + offset_`, `_classes.py:1801`), and no oracle pins the values (target df `[-0.01,0.0,-0.01,-0.01,0.0,0.0,0.29]`, score_samples `[0.0,0.01,0.0,0.0,0.01,0.01,0.3]`). |
| REQ-5 (predict +1/-1) | NOT-STARTED | open prereq blocker #650. `fn predict in one_class_svm.rs` returns `Array1<isize>` `+1`/`-1` from `val >= F::zero()`. sklearn/libsvm use the **strict** `(sum > 0) ? +1 : -1` (`svm.cpp:2837-2838`) — a divergence at exactly `decision == 0` (libsvm `-1`, ferrolearn `+1`). The predict structure is otherwise correct, but (a) the `>=` vs `>` boundary diverges and (b) predict inherits REQ-1's scaling divergence (the sign of `decision − rho` is scale-invariant for `ν·n > 0`, so labels may still match the oracle despite the value divergence — the critic should pin the labels `[-1,1,-1,-1,1,1,1]` AND a constructed exact-zero case). |
| REQ-6 (constructor params/defaults) | NOT-STARTED | open prereq blocker #651. `OneClassSVM::new` defaults `nu=0.5` (matches), `tol=1e-3` (matches), but `max_iter=10000` (sklearn `-1` = no limit) and `cache_size=1024` (sklearn `200`, and unused — no kernel cache in this module). `nu`/`kernel`/`degree`/`gamma`/`coef0`/`shrinking` surface: `nu` is present; `kernel`/`degree`/`coef0` are the documented R-DEV-7 type-parameter design difference (kernel = type param `K`, set by construction) as in `svm.rs`; `gamma` resolution is REQ-2; **`shrinking` is absent** (libsvm-irrelevant since this SMO has no shrinking heuristic, but the API field is missing — sklearn accepts it). No oracle/test pins the default surface. |
