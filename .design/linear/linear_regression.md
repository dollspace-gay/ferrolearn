...
| REQ-5 (positive / NNLS) | SHIPPED | #371: `LinearRegression<F>` gains `pub positive: bool` (default `false`, sklearn ctor `_base.py:574`) + `with_positive(bool)` builder. The coefficient solve in `fit_with_sample_weight` routes through `solve_coef in linear_regression.rs`, which calls `crate::linalg::nnls` — a Lawson-Hanson active-set non-negative least-squares solver (passive-set unconstrained LS via `solve_lstsq` on the passive columns, outer iterations capped at `3·n`) — instead of `solve_lstsq` when `positive`, on the SAME centered + `√w`-rescaled design, mirroring sklearn `self.coef_ = optimize.nnls(X, y)[0]` (`_base.py:647`). Intercept recovered identically (`y_off − X_off·coef`, `_set_intercept`, `_base.py:692`); `rank_`/`singular_` taken from the design's `solve_lstsq` SVD (sklearn leaves them unset on the positive path — documented analog). `positive=false` byte-identical to unconstrained OLS. Oracle-verified vs live sklearn: `coef_=[2.03571429, 0.0]`, `intercept_=-1.46428571` (`linreg_positive_matches_sklearn`; differs from unconstrained `[2.25, -0.75]`), raw `nnls(X,y)=[1.34210526, 0.0]` (`linreg_positive_no_intercept_matches_sklearn`, `nnls_matches_scipy`); byte-identical guard `linreg_positive_false_unchanged`. |
