...
| REQ-6 (multi-output 2-D Y) | NOT-STARTED | open prereq blocker #372. `Fit<Array2<F>, Array1<F>>` accepts only a 1-D `y` (`Array1<F>`); there is no `Array2` target arm, so the 2-D `coef_` of shape `(n_targets, n_features)` (`_base.py:499`) cannot be produced. |
| REQ-7 (sample_weight) | NOT-STARTED | open prereq blocker #373. `Fit::fit(&self, x, y)` has no `sample_weight` argument and performs no `√w` row-rescaling (`_rescale_data`, `_base.py:641`), so weighted OLS is unsupported. |
