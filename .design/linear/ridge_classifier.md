...
| REQ-7b (class_weight / solver+solver_) | NOT-STARTED | open prereq blocker #393 (positive done — see REQ-7a). `RidgeClassifier<F>` has no `class_weight` (sklearn `_ridge.py:1398`), no `solver` selection or `solver_` attribute (`_ridge.py:1406-1484`) — the unconstrained fit still uses the dense centered closed-form path. |
