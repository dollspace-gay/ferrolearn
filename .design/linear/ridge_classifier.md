...
`fit_intercept` by centering, and `coef_`/`intercept_`/`classes_` introspection. The separate
cross-validated `RidgeClassifierCV` estimator now ships in the sibling module
`ridge_classifier_cv.rs` (REQ-10, shared-alpha leave-one-out GCV over the binarized indicator
targets). This module still does NOT cover the `solver` variants and the `solver_` attribute,
or the ferray substrate.
...
| REQ-10 (RidgeClassifierCV) | SHIPPED | New module `ridge_classifier_cv.rs`: `RidgeClassifierCV<F>`/`FittedRidgeClassifierCV<F>` mirror `class RidgeClassifierCV` (`_ridge.py:2676`). `impl Fit<Array2<F>, Array1<usize>> for RidgeClassifierCV` in `ridge_classifier_cv.rs` binarizes `y` to a `{-1,+1}` indicator (binary single column / multiclass one-hot, mirroring `LabelBinarizer(pos_label=1, neg_label=-1)`, `_ridge.py:1300-1301`), then `fn select_alpha_gcv in ridge_classifier_cv.rs` selects ONE shared `alpha` by leave-one-out GCV over the binarized multi-target problem — the closed-form LOO errors `(c / G_inverse_diag)²` summed over EVERY indicator column AND sample against the SAME shared decomposition (hat-matrix diagonal depends only on `X+alpha`), mirroring `_RidgeGCV` (`_ridge.py:1688`) with `scoring=None` → `-squared_errors.mean()` over the `(n_samples, n_y)` errors (`_ridge.py:2148-2150` + `_score_without_scorer` `:2211-2218`); `fn gcv_scores_svd`/`fn gcv_scores_eigen in ridge_classifier_cv.rs` REPLICATE `ridge_cv.rs`'s verified 1-D `_RidgeGCV` centering/intercept-cancellation, extended over the indicator columns. The selected `alpha_` drives a multi-output `Ridge::fit` refit, stored as `coef_` `(n_classes_or_1, n_features)` + per-class `intercept_`; `impl Predict` does binary sign (`> 0`, `_base.py:384`) / multiclass argmax. `impl HasCoefficients`/`HasClasses` present. Non-test production consumer: crate-root re-export `pub use ridge_classifier_cv::{RidgeClassifierCV, FittedRidgeClassifierCV}` in `ferrolearn-linear/src/lib.rs` (the grandfathered R-DEFER-1 / S5 re-export boundary — same path by which the sibling CV estimator `RidgeCV` ships; `RidgeCV` likewise has no `ferrolearn-python` registration). Verification (live sklearn 1.5.2, R-CHAR-3): binary `RidgeClassifierCV(alphas=[0.1,1,10])` on `X=[[1,2],[2,1],[3,1],[1,3],[2,2],[6,5],[5,6],[7,7]]`, `y=[0,0,0,0,0,1,1,1]` → `alpha_=10.0`, `coef_ [[0.197492163, 0.197492163]]`, `intercept_ [-1.5830721003]`, `predict [0,0,0,0,0,1,1,1]`; multiclass `y=[0,0,1,1,2,2,1,0]` → `alpha_=10.0`, `coef_` `(3,2)` `[[-0.0031348,-0.0031348],[-0.07817398,0.04682602],[0.08130878,-0.04369122]]`, `predict [1,0,0,1,1,0,1,0]`. Both binary AND multiclass shared-alpha GCV match the oracle to <1e-6 (no binary-only fallback). Tests `ridge_classifier_cv_binary_matches_sklearn`, `ridge_classifier_cv_multiclass_matches_sklearn`, `ridge_classifier_cv_single_class_errors`, `ridge_classifier_cv_selects_from_alphas` PASS. Spillover (not in this manifest): `ferrolearn-python` registration of `RsRidgeClassifierCV` is a follow-up requiring a binding-crate manifest expansion. |
