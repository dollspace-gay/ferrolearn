...
| REQ | Status | Evidence |
|---|---|---|
| REQ-1 (fit + decision_function parity) | NOT-STARTED | open prereq blocker #575. Impl `fn fit in qda.rs` computes per-class mean + `(n_k-1)` covariance and `fn cholesky_inv_and_logdet` yields `cov_inv`/`log_det`; `fn decision_function in qda.rs` builds `out[[i,c]] = -half*model.log_det - half*mahal + model.log_prior`. The live oracle confirms this **inversion** path matches sklearn's **SVD** `_decision_function` to ~1e-14 on full-rank data (reg 0/0.3/0.5; see Verification), so the algorithm is correct — BUT no test pins the `(n,n_classes)` raw decision against the live oracle: `test_binary_classification`/`test_multiclass_classification in qda.rs` assert only an accuracy floor, and `conformance_qda` (the excluded fixture suite) does not count (R-CHAR-3, R-HONEST-3). |
...
| REQ-10 (tol / rank-collinearity) | NOT-STARTED | open prereq blocker #583. `QDA<F>` has no `tol` field and no collinearity warning. ferrolearn's `fn cholesky_inv_and_logdet in qda.rs` returns `FerroError::NumericalInstability { message: "covariance matrix is not positive definite" }` when `s <= F::zero()` — so a rank-deficient/collinear class **fails** where sklearn's SVD path succeeds (with a "Variables are collinear" warning) and still produces predictions (`discriminant_analysis.py:945-947`; live oracle: a collinear class yields `scalings_[c] == [2.0, 0.0]` and `predict` still returns). Behavioral divergence on rank-deficient input (R-DEV-1). |
| REQ-11 (fitted attributes) | NOT-STARTED | open prereq blocker #584. `FittedQDA` exposes only `fn means in qda.rs` (returns `Vec<&Array1<F>>`) and `HasClasses`. There are no `priors_`, `scalings_`, `rotations_`, or `covariance_` accessors — and because ferrolearn inverts the covariance rather than computing the SVD, `scalings_`/`rotations_` are not even materialized. sklearn exposes all five (`discriminant_analysis.py:810-841`). |
| REQ-12 (ferray substrate) | NOT-STARTED | open prereq blocker #585. `qda.rs` imports `ndarray::{Array1, Array2, ScalarOperand}` and operates on `Array2<F>`/`Array1<F>`; the Cholesky inverse/log-det is hand-rolled on `ndarray`, not `ferray::linalg`. Consistent with the crate-wide substrate deferral (cf. `isotonic.md` REQ-11 #572, `glm.md`, ridge #391). |
