...
| REQ-4 (predict_log_proba) | SHIPPED | #578. Impl `fn predict_log_proba in qda.rs` returns `crate::log_proba(&self.predict_proba(x)?)` — the elementwise log of `predict_proba`, mirroring sklearn `probas_ = self.predict_proba(X); return np.log(probas_)` (`discriminant_analysis.py:1058-1059`). Consumer: `fn predict_proba in qda.rs` (production) is shared with the `Predict` impl. Verified: `qda_predict_log_proba in tests/divergence_qda_fit.rs` pins `predict_log_proba(X)` on the OVERLAPPING 3-class `mc_x`/`mc_y` dataset (every class proba in `(0,1)` ⇒ all log-probas FINITE, no `log(0)=-inf`) to the live `Q().predict_log_proba(X)` oracle (<1e-6, R-CHAR-3). |
...
| REQ-6 (priors None + provided) | SHIPPED | #580. `QDA<F>` gains a `priors: Option<Array1<F>>` field (default `None`) + `with_priors in qda.rs` builder, mirroring sklearn's `priors` constructor arg (`_parameter_constraints` `"priors": ["array-like", None]`, `discriminant_analysis.py:341`; `__init__` default `priors=None`, `:351,359`). `fn fit in qda.rs` resolves `priors_`: `None` → empirical `n_k/n` per class (bit-identical to the previous `(n_k_f/n_f).ln()` argument, so the empirical path is byte-identical and the 6 existing pins stay green); `Some(p)` → `p` **verbatim** (no renormalization, no non-negativity/sum check), matching `self.priors_ = np.array(self.priors)` (`:921-924`). The resolved prior enters as `log_prior = priors[k].ln()` → `fn raw_decision` additive `+ log(prior_k)` (`:976`); a zero entry gives `log(0) = -inf` and a negative gives `NaN`, no special-casing. **R-DEV-4 deviation**: a wrong-length `priors` (`p.len() != n_classes`) returns `FerroError::ShapeMismatch` — sklearn silently mis-indexes (a CPython/numpy footgun Rust eliminates). Consumer: `fn fit` reads `self.priors` (production); `FittedQDA::priors in qda.rs` exposes the resolved `priors_`. Verified: `qda_provided_priors in tests/divergence_qda_fit.rs` pins `predict_proba` with `priors=[0.9,0.1]` to the live oracle (<1e-6) + `qda_priors_length_mismatch` pins the R-DEV-4 error. |
...
| REQ-9 (store_covariance / covariance_) | SHIPPED | #582. `QDA<F>` gains a `store_covariance: bool` field (default `false`, sklearn's default) + `with_store_covariance in qda.rs` builder, mirroring sklearn's `store_covariance` constructor arg (`discriminant_analysis.py:353,361`; `_parameter_constraints` `"store_covariance": ["boolean"]`, `:342`). `fn fit in qda.rs` reconstructs per-class `covariance_ = Rₖ·diag(scalingsₖ)·Rₖᵀ` from the **regularized** scalings (matching `cov.append(np.dot(S2 * Vt.T, Vt))` where `S2` is the regularized blend, `discriminant_analysis.py:949,951-952`) ONLY when `store_covariance` is set (`:928-929,955-956` — the `covariance_` attribute exists only under `store_covariance=True`). Consumer: `fn fit` reads `self.store_covariance` (production); `FittedQDA::covariance in qda.rs` exposes `Option<&[Array2<F>]>` (`None` when unset, matching sklearn's missing attribute). Verified: `qda_store_covariance in tests/divergence_qda_fit.rs` pins per-class `covariance()` to the live `Q(store_covariance=True).covariance_` oracle (<1e-9) and asserts `with_store_covariance(false)`/default → `covariance() == None` (R-CHAR-3). |
