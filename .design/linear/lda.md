...
| REQ-7 (priors None + provided) | SHIPPED (empirical) | `fn fit in lda.rs` resolves `priors_ = n_k/n` per class (`discriminant_analysis.py:601-603`) and `FittedLDA::priors` exposes them; the priors enter the affine decision through `intercept_ = -½Σcoef² + log(priors_)` (`:557`) and `xbar_ = priors_ @ means_` (`:517`). Consumer: `fn fit` reads them, `fn decision_function` consumes `intercept_`, `fn transform` consumes `xbar_`. R-CHAR-3 pin `lda_imbalanced_priors_predict` (priors `[0.9091,0.0909]` flip the label). The **provided**-`priors` constructor parameter (array, renormalize-with-warning if `|Σ-1|>1e-5`, reject negatives) is a follow-up tracked at #593 — the empirical-prior root cause of the REQ-1/2/3 divergence is shipped here. |
