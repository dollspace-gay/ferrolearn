...
| REQ-9 (lsqr solver) | SHIPPED | `Solver::Lsqr` (`LDA::with_solver`) dispatches `fn fit` to `fn solve_lstsq` (sklearn `_solve_lstsq`, `discriminant_analysis.py:365-419`): `covariance_ = Σ_k priors_[k]·cov(X_k)` (always populated for lsqr, `:413`); `coef_ = lstsq(covariance_, means_.T)[0].T` (`:416`) via `fn lstsq_multi` → `ferray::linalg::lstsq` (multi-RHS); `intercept_ = -½·diag(means_ @ coef_.T) + log(priors_)` (`:417-418`). No `transform` for lsqr (`max_components=0`, mirrors the `NotImplementedError` at `:676-679`). Binary `coef_[1]-coef_[0]` collapse deferred to #600 (coef_ stays `(n_classes, n_features)`, matching the svd path). Consumer: `fn fit` dispatches on `self.solver`; `Predict`/`predict_proba for FittedLDA`. Test `lda_lsqr_solver` (collapsed coef `[14.7368…, 14.7368…]`, predict/predict_proba) <1e-6 vs live `L(solver='lsqr')`. #595. |
| REQ-10 (eigen solver) | SHIPPED | `Solver::Eigen` (`LDA::with_solver`) dispatches `fn fit` to `fn solve_eigen in lda.rs` (sklearn `_solve_eigen`, `discriminant_analysis.py:421-485`): `Sw = covariance_ = Σ_k priors_[k]·cov(X_k)` (within scatter, `:467-471`); `St = _cov(WHOLE X, shrinkage)` (total scatter, `:472`); `Sb = St - Sw` (`:473`). The generalized symmetric-definite `eigh(Sb, Sw)` (`:475`) is reduced to a STANDARD symmetric eigenproblem via the Cholesky factor of `Sw` (ferray exposes `eigh`/`cholesky` but no generalized solver): `Sw = L·Lᵀ` (`fn cholesky_lower` → `ferray::linalg::cholesky`), `M = L⁻¹·Sb·L⁻ᵀ` (`fn matrix_inverse` → `ferray::linalg::inv`) symmetrized `(M+Mᵀ)/2`, `(evals, W) = eigh(M)` (`fn eigh_sym` → `ferray::linalg::eigh`, ascending), generalized `evecs = L⁻ᵀ·W` sorted DESCENDING by eigenvalue (`:479`). `explained_variance_ratio_ = sort(evals/Σevals)[::-1][:max_components]` (`:476-478`); `scalings_ = evecs` (all columns, `:481`); `coef_ = (means_@evecs)@evecs.T` (`:482`, sign/order-invariant so it matches sklearn despite the eigenvector ambiguity); `intercept_ = -½·diag(means_@coef_.T) + log(priors_)` (`:483-485`). Supports `shrinkage`; `transform` is the un-centered `X @ scalings_[:, :max_components]` (eigen has no `xbar_`, `:687`). Consumer: `fn fit` dispatches on `self.solver`; `Predict`/`predict_proba`/`Transform for FittedLDA`. Tests `lda_eigen_solver` (collapsed coef `[14.7368…, 14.7368…]`, evr `[1.0]`, predict/predict_proba) + `lda_eigen_shrinkage` (`Fixed(0.5)` collapsed `[12.043…, 12.043…]`) <1e-6 vs live `L(solver='eigen')`. #596. |
| REQ-11 (shrinkage None/auto/float) | SHIPPED | `Shrinkage::{None, Auto, Fixed(F)}` (`LDA::with_shrinkage`) drives `fn cov_shrunk` (sklearn `_cov`, `discriminant_analysis.py:36-93`) inside `fn solve_lstsq`: `None` → ML empirical cov (`fn empirical_covariance`, `np.cov(...,bias=1)`); `Fixed(s)` → `(1-s)·emp + s·(trace/p)·I` (`shrunk_covariance`, `_shrunk_covariance.py:153-156`), validated `0 ≤ s ≤ 1` (`Interval(Real,0,1,closed=both)`, `:339`); `Auto` → analytical Ledoit-Wolf (`fn ledoit_wolf_shrinkage`, transcribed from `_shrunk_covariance.py:365-401`) on StandardScaler-standardized data then rescaled (`:70-75`). `Solver::Svd` + non-`None` shrinkage → `InvalidParameter("shrinkage not supported with svd solver")` mirroring `NotImplementedError` (`:628-629`). Consumer: `fn fit`/`fn solve_lstsq` read `self.shrinkage`. Tests `lda_shrinkage_fixed` (`[12.043…]`), `lda_shrinkage_auto` (`[11.3706…]`, validates the Ledoit-Wolf transcription), `lda_svd_shrinkage_rejected` <1e-6 vs the live oracle. #597. |
