...
program** with HiGHS (`scipy.optimize.linprog`). ferrolearn instead fits via
**IRLS on a smoothed pinball loss**, which does not reach the LP vertex optimum
— the central divergence tracked by issue #340. Parameter handling, the
quantile asymmetry, prediction, and `HasCoefficients` are in place; the LP
solver, the LP-as-variable intercept, `n_iter_`, the `solver`/`solver_options`
params, and the ferray substrate are not.
...
| REQ | Status | Evidence |
|---|---|---|
| REQ-1 (LP-based fit) | NOT-STARTED | open prereq blockers #340 (LP/HiGHS solver) and #506 (intercept as LP variable). ferrolearn fits via IRLS (`fn fit in quantile_regressor.rs`, the `for _iter in 0..self.max_iter` loop calling `weighted_l1_solve`), not the LP that sklearn builds at `_quantile.py:212-277` (`c`/`A_eq`/`b_eq` then `linprog(method=solver)`). Live oracle (`quantile=0.5, alpha=0.0`) sklearn `coef=[0.8992,2.0000,-0.8874]`, `intercept=0.3243`; ferrolearn `coef=[0.8845,1.9848,-0.8941]`, `intercept=0.2989` — close on coefficients here but the intercept (via centering, see #506) is wrong, and the gap widens under regularization. |
| REQ-2 (alpha·n_samples scaling) | NOT-STARTED | open prereq blocker #332. The scaling **factor** is present and correct — `weighted_l1_solve in quantile_regressor.rs` computes `let scaled_alpha = alpha * n_f;` (`n_f = n_samples`), matching sklearn `_quantile.py:182` (`alpha = np.sum(sample_weight) * self.alpha`). But it is applied as an IRLS-linearized ridge-like diagonal penalty `scaled_alpha / max(|w_j|, eps)`, not the LP's exact L1 cost, so it cannot reach sklearn's sparse optimum: live oracle `alpha=1.0, quantile=0.5` sklearn `coef=[0,0,0]` (exact zeros) vs ferrolearn `coef≈[1.7e-9, 2.2e-6, -5.5e-10]`. Correct meaning of `alpha` is gated on the LP (#340/#332). |
