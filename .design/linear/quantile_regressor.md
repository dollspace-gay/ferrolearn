...
quantile asymmetry, prediction, `HasCoefficients`, the `n_iter_` attribute
(#507, the two-phase-simplex pivot count — R-DEV-7: not scipy's `result.nit`),
and the `solver`/`solver_options` constructor params (#508) are in place; the
ferray substrate (#509) is not yet.
...
| REQ-6 (n_iter_) | SHIPPED | impl: `mod lp in quantile_regressor.rs` now returns the total two-phase primal-simplex PIVOT count (`LpStatus::Optimal(Vec<f64>, usize)`; `optimize` returns `Some(pivots)`, `solve` sums phase 1 + phase 2); `fn fit` stores it on `FittedQuantileRegressor` and `pub fn n_iter in quantile_regressor.rs` exposes it — the ferrolearn analog of sklearn `self.n_iter_ = result.nit` (`_quantile.py:300`). R-DEV-7: ferrolearn's from-scratch two-phase simplex is NOT scipy's HiGHS, so the pivot count does NOT equal sklearn's `n_iter_` (HiGHS reports 31 for "highs"/"highs-ds", 8 for "highs-ipm" on the AC-1 dataset); the SHIPPED contract is the cross-solver invariant: a positive int `<= max_iter`, deterministic. Non-test consumer: `RsQuantileRegressor::n_iter_` getter in `ferrolearn-python/src/extras.rs` → `_extras.py::QuantileRegressor.fit` sets `self.n_iter_ = int(self._rs.n_iter_)`. Verification: `cargo test -p ferrolearn-linear --test divergence_quantile_fit` (`n_iter_positive_and_bounded_on_oracle`, `n_iter_deterministic_on_oracle`) + `tests/divergence_extras.py::test_quantile_n_iter_contract` (pytest, sklearn 1.5.2). |
| REQ-7 (solver / solver_options) | SHIPPED | impl: `QuantileRegressor in quantile_regressor.rs` adds `solver: String` (default `"highs"`, mirroring `_quantile.py:134`) + `solver_options: Option<HashMap<String, f64>>` (default `None`, `_quantile.py:135`), with `with_solver`/`with_solver_options` builders. `fn fit` validates them mirroring sklearn's `_parameter_constraints["solver"]` (`_quantile.py:114-124`, `StrOptions({"highs-ds","highs-ipm","highs","interior-point","revised simplex"})`): the HiGHS family + `"revised simplex"` reach the same unique LP vertex → ferrolearn's two-phase simplex (identical coef_, verified); `"interior-point"` → `InvalidParameter` (mirrors sklearn's `ValueError`, "removed in SciPy 1.11.0", `_quantile.py:196-199`); any other string → `InvalidParameter` (mirrors sklearn's `InvalidParameterError` ⊂ `ValueError`). `solver_options` `None`/empty are accepted+ignored (HiGHS tuning knobs that do not change the LP optimum, `_quantile.py:206`); a non-empty map → `InvalidParameter("solver_options")`, marshalled to `NotImplementedError` (#508 — ferrolearn's simplex has no HiGHS tuning surface). Non-test consumer: hand-written `RsQuantileRegressor` in `ferrolearn-python/src/extras.rs` (replacing the `py_regressor!` macro) surfaces `solver`/`solver_options`, with `quantile_fit_err` mapping `solver`→`ValueError`, non-empty `solver_options`→`NotImplementedError`; `_extras.py::QuantileRegressor` passes them through. Verification: `cargo test -p ferrolearn-linear` (`test_highs_family_solvers_give_identical_coef`, `test_invalid_solver_string_rejected`, `test_interior_point_rejected`, `test_solver_options_none_and_empty_accepted`, `test_nonempty_solver_options_rejected`, + `solver_family_gives_identical_coef_and_matches_sklearn`/`invalid_solver_string_is_rejected`/`interior_point_solver_is_rejected` in `divergence_quantile_fit`) + `tests/divergence_extras.py::test_quantile_{default_solver_predict,highs_family_solvers_give_identical_predict,invalid_solver_raises_valueerror,interior_point_raises_valueerror,solver_options_none_and_empty_accepted,nonempty_solver_options_raises_notimplemented}` (pytest, live sklearn 1.5.2). |
...
REQ-1, REQ-2 (closed #340/#332/#506) and REQ-6/REQ-7 (closed #507/#508) now have
green verification against the live sklearn 1.5.2 contract (see the REQ-status
table evidence). REQ-8 (ferray substrate) remains NOT-STARTED, gated on blocker
#509. REQ-1 closes only when a quantile-regression LP solver lands
