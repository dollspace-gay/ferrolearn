...
| REQ | Status | Evidence |
|---|---|---|
| REQ-1 (classifier losses hinge/log/modified_huber/squared_error) | SHIPPED | impl `struct Hinge`, `struct LogLoss`, `struct ModifiedHuber`, `struct SquaredError` (`Loss` impls in `sgd.rs`) match `_sgd_fast.pyx.tp`: Hinge `loss`/`gradient` mirror `:216-226` (`thr-z`/`-y` for `z<thr`); LogLoss mirrors `:267-283` with the same `±18` overflow guard and `-y·exp(-z)/(1+exp(-z))` gradient; ModifiedHuber mirrors `:178-194` (`(1-z)^2` mid-region, `-4z`/`-4y` tail, `-2y(1-z)` mid-gradient); SquaredError mirrors `:291-295` (`0.5(p-y)^2`, grad `p-y`). Consumed in production via `fn dispatch_train_binary in sgd.rs` → `fn fit (Fit for SGDClassifier)` → `impl PipelineEstimator for SGDClassifier in sgd.rs` (`Pipeline::fit` boxes it as `FittedPipelineEstimator`). Tests: `test_hinge_loss_*`, `test_log_loss_*`, `test_modified_huber_loss`, `test_squared_error_loss`. NOTE these unit tests assert hand-written constants, not the live `py_dloss` oracle (R-CHAR-3 gap) — the formulas are textually identical to `_sgd_fast` so this is SHIPPED, but the characterization test should be re-pinned against `LossFunction.py_loss`. |
...
| REQ-4 (L2 penalty update = wscale shrink) | NOT-STARTED | open prereq blocker #525. `fn train_binary_sgd in sgd.rs` applies L2 *inline per touched feature* — `weights[j] -= eta*(grad*xi[j] + alpha*weights[j])` — folding the penalty into the gradient step on only the features in `x`. sklearn applies a *global* multiplicative `wscale` shrink to the whole vector before the gradient add: `w.scale(max(0, 1 - (1-l1_ratio)*eta*alpha))` (`_sgd_fast.pyx.tp:632-635`). For dense `x` with all-nonzero features the two differ at O(eta²·alpha) and on the `max(0,·)` clamp; this is a numerical divergence (R-DEV-1). |
| REQ-5 (l1/elasticnet + l1_ratio) | NOT-STARTED | open prereq blocker #526. No `penalty`/`l1_ratio` fields on `SGDClassifier`/`SGDRegressor` (`sgd.rs`); the L2-only update in `train_binary_sgd`/`train_regressor_sgd` has no `u`/`q` cumulative-penalty vector. sklearn's `PENALTY_TYPES` (`:55`) supports `l1`/`elasticnet` via `l1penalty` truncated gradient (`_sgd_fast.pyx.tp:656-658,750-778`). |
...
| REQ-7 (optimal schedule t0 init) | NOT-STARTED | open prereq blocker #527. `fn compute_lr in sgd.rs` returns `Optimal => 1/(alpha*t)`, omitting the `optimal_init` offset: sklearn computes `typw=sqrt(1/sqrt(alpha))`, `initial_eta0=typw/max(1,dloss(1,-typw))`, `optimal_init=1/(initial_eta0*alpha)` and uses `eta=1/(alpha*(optimal_init + t - 1))` (`_sgd_fast.pyx.tp:565-570,591-592`). The current form has the wrong value at every `t` (e.g. at `t=1` sklearn gives `initial_eta0`, ferrolearn gives `1/(alpha*1)`). |
...
| REQ-9 (default params per estimator) | NOT-STARTED | open prereq blocker #529. `fn new in sgd.rs` for `SGDClassifier` sets `learning_rate=InvScaling, eta0=0.01, power_t=0.25`; sklearn's `SGDClassifier.__init__` defaults are `learning_rate="optimal", eta0=0.0, power_t=0.5` (`_stochastic_gradient.py:1242-1244`). (SGDRegressor's `invscaling/0.01/0.25` DO match `:2042-2044`.) Also missing fields entirely: `penalty`, `l1_ratio`, `fit_intercept`, `shuffle`, `epsilon`, `n_iter_no_change`, `early_stopping`, `validation_fraction`, `average`, `warm_start`, `class_weight`, `C`. |
...
| REQ-13 (early_stopping + validation_fraction) | NOT-STARTED | open prereq blocker #533. No `early_stopping`/`validation_fraction` fields and no validation split / `_ValidationScoreCallback` analog (`_stochastic_gradient.py:63-79`, `:114-115`); the kernel has no `validation_mask`/`early_stopping` branch (`_sgd_fast.pyx.tp:586-588,678-687`). |
...
| REQ-18 (SGDOneClassSVM) | NOT-STARTED | open prereq blocker #536 (builder territory). The estimator is entirely absent from `sgd.rs` — no `SGDOneClassSVM` type, no `offset_`, no `nu`, no one-class intercept update (`intercept_update -= 2*eta*alpha`, `_sgd_fast.pyx.tp:641-642`; class at `_stochastic_gradient.py:2084-2278`). Routed parity op with no implementation. |
| REQ-19 (anti-pattern cleanup) | NOT-STARTED | open prereq blocker #537. `fn compute_lr in sgd.rs` contains `LearningRateSchedule::_Phantom(_) => unreachable!()` (R-APG-1 forbids `unreachable!()` outside `#[cfg(test)]`); the loss kernels use `F::from(18.0).unwrap()`, `F::from(1e18).unwrap()`, `F::from(0.5).unwrap()` etc. in production (`Loss` impls, `compute_lr`), which are `.unwrap()` calls outside `#[cfg(test)]` (R-CODE-2 / R-APG-1). The `_Phantom` variant should be removed and constants built infallibly. |
...
|---|---|---|---|
| L2 update | global `wscale` shrink before grad (`:632-635`) | inline per-feature `eta*alpha*w_j` | REQ-4 |
| optimal η | `1/(alpha*(optimal_init+t-1))` (`:591-592`) | `1/(alpha*t)` | REQ-7 |
...
| clf defaults | optimal/0.0/0.5 (`:1242-1244`) | invscaling/0.01/0.25 | REQ-9 |
| dloss clip | `±1e12` (`:616-619`) | none | REQ-10/cleanup |
| penalty | l2/l1/elasticnet (`:55`) | l2 only | REQ-5 |
| OneClassSVM | full estimator (`:2084`) | absent | REQ-18 |
