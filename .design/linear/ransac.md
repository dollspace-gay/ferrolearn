...
| REQ-8 (loss='squared_error') | NOT-STARTED | open prereq blocker #516. ferrolearn hardcodes absolute error (`fn fit in ransac.rs`, `ransac.rs:289`: `let residual = (preds[i] - y[i]).abs();`) with no `loss` parameter. sklearn supports `'absolute_error'` (default), `'squared_error'` (`(y_true - y_pred)**2`), and callables (`_ransac.py:405-421`); the `loss` constructor param is constrained by `StrOptions({"absolute_error", "squared_error"})` (`_ransac.py:284`). |
...
| REQ-12 (min_samples float fraction) | NOT-STARTED | open prereq blocker #520. ferrolearn's `min_samples: Option<usize>` (`ransac.rs:63`) accepts only integer counts (`with_min_samples(min_samples: usize)`, `ransac.rs:92`). sklearn accepts a float fraction in `(0,1)` → `ceil(min_samples * n_samples)` (`_ransac.py:389-390`), constrained by `Interval(RealNotInt, 0, 1, closed="both")` (`_ransac.py:264`). |
