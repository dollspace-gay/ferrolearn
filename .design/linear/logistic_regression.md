...
| REQ-12 (class_weight) | NOT-STARTED | open prereq blocker #445. No `class_weight` field; samples are unweighted. |
...
| REQ-17 (n_iter_) | NOT-STARTED | open prereq blocker #450. `FittedLogisticRegression` does not store the optimizer iteration count; `LbfgsOptimizer::minimize` discards it. |
| REQ-18 (sample_weight) | NOT-STARTED | open prereq blocker #451. `fit` takes no `sample_weight`. |
| REQ-19 (random_state/n_jobs) | NOT-STARTED | open prereq blocker #452. Neither parameter exists (lbfgs is deterministic, but the API surface is missing). |
