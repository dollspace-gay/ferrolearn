...
The sklearn helpers `orthogonal_mp`, `orthogonal_mp_gram`, `ridge_regression`, and `l1_min_c` are also surfaced from this
...
1. **Module declarations + re-export block (REQ-1).** `pub mod` for every estimator module
   ...
   **Omissions (estimators ferrolearn lacks — documented, not a `lib.rs` gap):** relative to
   ...
   free functions. Those are missing-estimator gaps owned by
