...
The divergences are: the **default `out_of_bounds`** (ferrolearn defaults to
`Clip`, sklearn defaults to `'nan'`), the absence of `increasing='auto'`
(Spearman `check_increasing`), `y_min`/`y_max` clipping, `sample_weight`
(weighted PAVA), `_make_unique` weighted duplicate-X collapse (duplicate X
values diverge), public fitted-attribute accessors
...
| REQ-4 (out_of_bounds {nan,clip,raise} + default nan) | NOT-STARTED | open prereq blocker #565. The three strategies exist as `pub enum OutOfBounds { Clip, Nan, Raise }` (`isotonic.rs:43-50`) and are applied in `fn predict_single` (`isotonic.rs:147-165`). But the **default diverges**: `fn IsotonicRegression::new` sets `out_of_bounds: OutOfBounds::Clip` (`isotonic.rs:82`) whereas sklearn defaults to `'nan'` (`isotonic.py:274` `out_of_bounds="nan"`; `_parameter_constraints` `StrOptions({"nan","clip","raise"})`, `isotonic.py:271`; live oracle `IsotonicRegression().out_of_bounds == 'nan'`, default-fit predict outside the domain → `NaN`). A user who constructs the default estimator gets clipped values where sklearn returns NaN — a default-ABI divergence (R-DEV-2). Also: ferrolearn's `Clip`/`Nan` interior path is correct, but sklearn's `'clip'` clamps `T` to `[X_min_, X_max_]` BEFORE interpolation (`isotonic.py:419-420`) — equivalent at the endpoints; the divergence pinned here is the default. |
...
| REQ-8 (_make_unique duplicate-X collapse) | NOT-STARTED | open prereq blocker #569. ferrolearn does NOT collapse duplicate X before PAVA — `pav_increasing` sorts by `x` only (`isotonic.rs:213`: `indices.sort_by(\|&a, &b\| xs[a].partial_cmp(&xs[b]))`, no secondary `y` key, no unique step) and the breakpoint emitter only skips re-pushing an already-present `result_x` (`isotonic.rs:266-273`), which collapses duplicate X to a block mean that does NOT match sklearn's pre-pool weighted collapse. sklearn does `np.lexsort((y, X))` then `_make_unique` (weighted mean of equal-X runs) BEFORE pooling (`isotonic.py:317-319`; live oracle: `X=[1,1,2,3], y=[1,3,2,4]` → `X_thresholds_ == [1,2,3]`, `y_thresholds_ == [2,2,4]`; ferrolearn computes `X=[1,2,3]`, `y≈[1,2.5,4]`). Divergent on any duplicate-X input. |
