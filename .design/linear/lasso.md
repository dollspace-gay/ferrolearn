...
| REQ-11 (n_iter_ / dual_gap_) | SHIPPED | `FittedLasso<F> in lasso.rs` now carries `n_iter`/`dual_gap` fields with `#[must_use]` getters `n_iter()`/`dual_gap()`, mirroring sklearn `Lasso.n_iter_` (`_coordinate_descent.py:1103`/`:827`) and `dual_gap_` (`:1108`/`:831`). `fn lasso_dual_gap in lasso.rs` implements the duality gap from `_cd_fast.pyx:216-247` (`XtA = Xcᵀ·R − β·w` with `β=0`, `l1_reg = α·n` per `_coordinate_descent.py:655`, branch on `dual_norm_XtA > l1_reg`) and divides by `n` to map sklearn's un-normalized `(1/2)` objective to ferrolearn's `(1/2n)` scaling; `Fit::fit` calls it on the (centered/raw) CD design with the final coef. HONEST NOTE: `n_iter_`'s VALUE differs from sklearn — ferrolearn's stopping criterion is max-coefficient-change, not sklearn's dual-gap (REQ-12 #412); the attribute is now EXPOSED and `dual_gap_` matches sklearn's formula and value (`lasso_dual_gap` → `0.0001170161` at the optimum vs sklearn `0.00011701482`). This REQ ships the ATTRIBUTES; exact `n_iter_` value-parity remains coupled to REQ-12. Verification (R-CHAR-3, numpy/sklearn oracle points): `cargo test -p ferrolearn-linear --lib lasso` — `lasso_dual_gap_formula_matches_numpy` (gap at `w=[0.5,1.0]`→`0.465888`, at `w=[0.66691036,1.46647171]`→`0.0001170161`), `lasso_fitted_dual_gap_and_n_iter`, `lasso_fields_dont_change_coef`. |
