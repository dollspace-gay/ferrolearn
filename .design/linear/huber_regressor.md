...
| REQ-9 (n_iter_) | SHIPPED | closes #499. `FittedHuberRegressor` carries an `n_iter` field set from the optimizer iteration count, exposed via `pub fn n_iter` in `huber_regressor.rs` — mirroring sklearn `self.n_iter_ = _check_optimize_result("lbfgs", opt_res, self.max_iter)` (`sklearn/linear_model/_huber.py:342`, i.e. `opt_res.nit`). `fn fit_with_sample_weight` now calls the ADDITIVE reporting overload `crate::optim::lbfgs::LbfgsOptimizer::minimize_reporting` (returns `(params, n_iters)`), extracted from a shared `fn minimize_inner`; `fn minimize` is byte-identically unchanged (still used by `logistic_regression.rs`, whose 26 tests stay green). R-DEV-7: ferrolearn's in-repo L-BFGS is NOT scipy's bounded L-BFGS-B, so the raw count is NOT asserted equal to sklearn's `n_iter_` — the SHIPPED contract is the honest ferrolearn count (positive int `<= max_iter`, deterministic, warm-refit `<` cold) which sklearn also exhibits. Live oracle (sklearn 1.5.2): cold `n_iter_=15`, warm `n_iter_=1`; ferrolearn: cold `n_iter_=13`, warm `n_iter_=1` (warm matches exactly; cold differs by the solver, documented). Non-test consumer: `ferrolearn-python/src/extras.rs` `RsHuberRegressor::n_iter_` getter → `_extras.py::HuberRegressor.fit` sets `self.n_iter_ = int(self._rs.n_iter_)`. Verification: Rust `cargo test -p ferrolearn-linear --test divergence_huber_fit` (`huber_n_iter_positive_bounded_and_deterministic`, `huber_warm_start_fewer_iters_than_cold`); pytest `test_huber_n_iter_attribute_positive_bounded`, `test_huber_n_iter_warm_start_fewer_than_cold_like_sklearn`. |
...
(sklearn `n_iter_`, REQ-9), exposed via `n_iter()` — the honest count of in-repo
