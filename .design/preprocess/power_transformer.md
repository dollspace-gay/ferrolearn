...
| REQ-6 (constant-feature → λ=1.0 skip + StandardScaler zero-scale handling) | NOT-STARTED | open prereq blocker #1347. `Fit::fit for PowerTransformer` always runs `brent_bounded` for every column — no `_is_constant_feature` skip — whereas sklearn sets `self.lambdas_[i] = 1.0` and `continue`s for a constant YJ feature (`_data.py:3299-3302`). `Transform::transform for FittedPowerTransformer` standardizes only `if s > F::zero()` (a constant transformed column is left UNSCALED, mean NOT subtracted), whereas sklearn's `StandardScaler(copy=False)` (`:3308-3314`) runs `_handle_zeros_in_scale` (scale 0 → 1) so a constant column maps subtract-mean → 0. |
| REQ-7 (NaN drop in optimize + box-cox check_positive) | NOT-STARTED | open prereq blocker #1348. `Fit::fit for PowerTransformer` builds the optimizer column via `x.column(j).iter().map(\|v\| v.to_f64().unwrap_or(0.0))` — **NaN → 0.0, NOT dropped** — biasing the MLE; there is no box-cox positivity check. sklearn drops NaN before optimizing (`x = x[~np.isnan(x)]`, `_data.py:3491`), masks NaN in box-cox (`x[~mask]`, `:3457`, all-NaN → `ValueError`), and rejects non-positive box-cox input (`np.nanmin(X) <= 0` raises, `:3524-3528`). |
