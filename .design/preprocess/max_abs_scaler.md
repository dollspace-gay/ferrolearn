...
| REQ-6 (scale_ / n_samples_seen_ attributes) | SHIPPED | `FittedMaxAbsScaler<F>` now materializes `pub(crate) scale_` (= `max_abs.mapv(|m| if m==0 {1} else {m})` = `_handle_zeros_in_scale(max_abs_)`, `_data.py:88`/`:1272`: equals `max_abs` on nonzero columns, `1.0` on all-zero columns) and `n_samples_seen_` (= n rows, `:1266`) in `Fit::fit`, with `#[must_use]` getters `scale()`/`n_samples_seen()`. `transform`/`inverse_transform` unchanged. Verification (live sklearn 1.5.2, R-CHAR-3, `X=[[1,0],[-3,0],[2,0]]` col1 all-zero): `max_abs_=[3,0]`, `scale_=[3.0,1.0]` (scale_[1]==1.0 while max_abs()[1]==0.0), `n_samples_seen_=3`. Tests `max_abs_scale_nsamples_match_sklearn`, `max_abs_scale_differs_from_max_abs_on_zero_col`. Boundary consumer: `FittedMaxAbsScaler` public API (grandfathered S5). |
