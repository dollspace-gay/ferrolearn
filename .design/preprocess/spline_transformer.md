...
| REQ-6 (error / parameter contracts, scoped + flagged DIVs) | SHIPPED (scoped) | `Fit::fit for SplineTransformer` returns `Err(FerroError::InsufficientSamples { required: 2, actual: n_samples, context: "SplineTransformer::fit" })` when `n_samples < 2`, `Err(InvalidParameter { name: "n_knots", reason: "n_knots must be at least 2" })` when `self.n_knots < 2`, and `Err(InvalidParameter { name: "degree", reason: "degree must be at least 1" })` when `self.degree == 0`; `Transform::transform for FittedSplineTransformer` returns `Err(ShapeMismatch { context: "FittedSplineTransformer::transform", .. })` on `x.ncols() != n_features`; `Transform::transform for SplineTransformer` (unfitted) returns `Err(InvalidParameter)`. Non-test consumer: these guards protect every instance reached through the crate re-export (`lib.rs`). Verification: `cargo test -p ferrolearn-preprocess spline` (`test_spline_insufficient_samples_error`, `test_spline_too_few_knots_error`, `test_spline_zero_degree_error`, `test_spline_shape_mismatch_error`, `test_spline_unfitted_error`) → green. **FLAG (candidate DIVs, NOT fixed here):** sklearn `_parameter_constraints` `n_knots: Interval(Integral, 2, None, closed="left")` (`_polynomial.py:717`) — ferrolearn's `n_knots < 2` rejection MATCHES; but `degree: Interval(Integral, 0, None, closed="left")` (`:718`) so `degree == 0` is VALID in sklearn (piecewise-constant) yet ferrolearn rejects it; and sklearn does NOT require `n_samples >= 2`. sklearn raises the cross-constraint `degree=0 and include_bias=False` error (`:326-330`) instead. These param-contract DIVs are tracked as open prereq blocker #1336. |
