...
| REQ-10 (center_ / scale_ attribute names + _handle_zeros semantics) | SHIPPED | `FittedRobustScaler<F>` now materializes `pub(crate) scale_` (= `iqr.mapv(|v| if v==0 {1} else {v})` = `_handle_zeros_in_scale(iqr)`, `_data.py:1634-1635`/`:88`) in `Fit::fit`, with `#[must_use]` getters `center()` (= `&median`, sklearn `center_`, `_data.py:1505-1514`) and `scale()` (= `&scale_`, sklearn `scale_`: equals `iqr` on non-constant columns, `1.0` on zero-IQR/constant columns). `median()`/`iqr()` (raw IQR) retained; `transform` unchanged. Verification (live sklearn 1.5.2, R-CHAR-3, col0 `[1,3,5,7,9]`, col1 constant `5`): `center_=[5,5]`, `scale_=[4,1]` (`scale()[1]==1.0` while `iqr()[1]==0.0`). Tests `robust_center_scale_match_sklearn`, `robust_scale_differs_from_iqr_on_constant_col`. |
