...
| REQ-4 (quantile_range ctor param) | NOT-STARTED | open prereq blocker #1249. `quantile_sorted` is called with hard-coded `0.25`/`0.75` in `fit`; `RobustScaler<F>` is a unit struct with no `quantile_range` field and no `0 <= q_min <= q_max <= 100` validation (sklearn `__init__` `:1567`, `:1604-1606`, per-column `nanpercentile(quantile_range)` `:1630`). Probe REQ-4 (`(10,90)` → `scale_=[62.7,24.0]`) unavailable. |
| REQ-5 (with_centering / with_scaling ctor params) | NOT-STARTED | open prereq blocker #1250. `RobustScaler<F>` is a unit struct with no params; `fit`/`transform` always center AND scale. No `with_centering`/`with_scaling` fields, no conditional `if with_centering: X -= center_` / `if with_scaling: X /= scale_` (`:1672-1675`), no `center_=None`/`scale_=None` paths (`:1616`,`:1640`), no `*`-only ctor (`:1562-1575`), no `_parameter_constraints`. Probe REQ-5 unavailable. |
...
| REQ-8 (robust_scale free fn + axis) | NOT-STARTED | open prereq blocker #1253. No standalone `robust_scale` in `robust_scaler.rs` or the crate; the estimator is hard-wired to per-column (`axis=0`) scaling with no `axis=1` row path (sklearn free fn `:1719`). Probe REQ-8 (`axis=1` row-scale) unavailable. |
