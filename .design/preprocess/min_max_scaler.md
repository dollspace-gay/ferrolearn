...
| REQ-5 (scale_/min_/data_range_/n_samples_seen_) | NOT-STARTED | open prereq blocker #1172. `FittedMinMaxScaler<F>` stores only `data_min`, `data_max`, `feature_range`; it recomputes the affine inline in `transform` and never materializes `scale_`/`min_`/`data_range_`/`n_samples_seen_` (sklearn `:508-514`). REQ-5 Probe attributes are inaccessible. |
| REQ-6 (inverse_transform) | NOT-STARTED | open prereq blocker #1173. No `inverse_transform` method on `FittedMinMaxScaler` (sklearn `:549-587`); the inverse affine `(x - fr[0])/scale_ + data_min` is not exposed. REQ-6 Probe round-trip unavailable. |
