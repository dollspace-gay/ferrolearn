...
| REQ-5 (var_ / scale_ / n_samples_seen_ + _handle_zeros) | NOT-STARTED | open prereq blocker #1192. `FittedStandardScaler<F>` stores only `mean` and `std` (= raw `sqrt(var)`); it never materializes `var_` (= std²), `scale_` (= `_handle_zeros_in_scale(sqrt(var_))`, which is `1` on constant columns — ferrolearn's `std` stays `0`), or `n_samples_seen_` (sklearn `:1013-1023`). REQ-5 Probe attributes are inaccessible, and `std() != scale_` on constant columns. |
| REQ-6 (with_mean / with_std / copy params) | NOT-STARTED | open prereq blocker #1193. `StandardScaler<F>` is a unit struct with no params; `fit`/`transform` always center AND scale. No `with_mean`/`with_std`/`copy` fields, no conditional `if with_mean: X -= mean_` / `if with_std: X /= scale_` (`:1064-1067`), no `*`-only ctor (`:835-838`), no `_parameter_constraints`. REQ-6 Probe (`with_mean=False`, `with_std=False`) unavailable. |
