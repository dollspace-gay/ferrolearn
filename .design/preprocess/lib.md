...
2. **The re-export block** — the `pub use` blocks (`:106-163`) that surface each
   ...
   `f_regression` free functions) at the crate root.
...
- REQ-1 (re-export boundary): the crate root surfaces, via the `pub use` block
  ...
  `chi2`/`f_classif`/`f_regression` scoring functions. The surfaced set is a
...
`chi2`, `f_classif`, `f_regression` (`:158-163`, `:136`, `:111-113`, `:138`,
`:153-155`).
ABSENT: `GenericUnivariateSelect`, `mutual_info_classif`,
`mutual_info_regression`, `r_regression`, `f_oneway`, and the `SelectorMixin`
base trait.

**`sklearn.feature_extraction.text` (`text.py:34-43`).**
PRESENT: `CountVectorizer`, `TfidfTransformer` (`:143-144`).
ABSENT: `HashingVectorizer`, `TfidfVectorizer` (the fused
count+tfidf vectorizer; ferrolearn ships the two stages separately but not the
fused convenience class).

**`sklearn.impute` (`impute/__init__.py:13`).**
PRESENT: `SimpleImputer`, `KNNImputer`, `IterativeImputer` (`:116`, `:132`,
`:130`). Note `IterativeImputer` is experimental in sklearn 1.5.2 (reached via
`enable_iterative_imputer`, hence not in the literal `__all__` at `:13`);
ferrolearn surfaces it unconditionally.
ABSENT: `MissingIndicator`.
...
- `cargo build -p ferrolearn-preprocess` — every `pub use` in `:106-163` resolves;
  ...
  boundary integrity).
- `cargo test -p ferrolearn-preprocess` — 429 lib unit tests + 44 doctests,
  0 failed (baseline `51f60dc0`).
