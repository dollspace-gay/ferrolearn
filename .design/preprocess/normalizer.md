...
| REQ-4 (normalize free function: axis / return_norm) | SHIPPED | `pub fn normalize<F: Float>(x, norm: NormType, axis: usize) -> Result<Array2<F>, FerroError>` and `pub fn normalize_with_norms<F: Float>(x, norm, axis) -> Result<(Array2<F>, Array1<F>), FerroError>` (the `return_norm=True` form). `axis=1` row-normalizes, `axis=0` column-normalizes (per-column divisor — sklearn transpose path `:1926-1942`/`:1971-1972`); `axis∉{0,1}` → `InvalidParameter`. Zero-norm vector left unchanged (`_handle_zeros`, `:1968`); `normalize_with_norms` returns the RAW per-axis norm vector (zero appears as `0.0`, sklearn `:1974-1975`). Reuses the REQ-2 input validation. Verification (live sklearn 1.5.2, R-CHAR-3, `X=[[1,2,2],[0,3,4]]`): l2/l1/max axis1, l2 axis0 `[[1,0.5547,0.4472],[0,0.832,0.894]]`, return_norm l2 `[3,5]` / l1 `[5,7]`. Tests `normalize_l2_axis1_matches_sklearn`, `normalize_l1_axis1_matches_sklearn`, `normalize_max_axis1_matches_sklearn`, `normalize_l2_axis0_matches_sklearn`, `normalize_return_norm_l2_and_l1`, `normalize_invalid_axis_errors`. |
