...
| REQ-2 (predict_proba / predict_log_proba) | NOT-STARTED | open prereq blocker (tracking #1690; critic to file specific). `FittedDummyClassifier` has NO `predict_proba`/`predict_log_proba` method — only `predict` (one-hot/argmax label) and `HasClasses`. sklearn returns the `class_prior_` row for `prior` (`sklearn/dummy.py:380-381`), one-hot for `most_frequent` (`:376-379`), `1/n_classes_` for `uniform` (`:387-389`) — live oracle: `prior`→`[[0.4,0.4,0.2],…]`, `most_frequent`→`[[1,0,0],…]`, `uniform`→`[[1/3,1/3,1/3]]` (Verification). This is the ONLY behavioral difference between the `Prior` and `MostFrequent` enum variants — their `predict` is byte-identical — so `Prior`'s distinguishing contract is entirely unshipped. Absent end-to-end. |
