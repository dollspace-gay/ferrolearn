...
`coef_`/`x_rotations_`/`y_rotations_`/`intercept_` attribute accessors (#1623), the
...
Count summary: **16 SHIPPED / 5 NOT-STARTED** (see `## REQ status`).
...
- REQ-4: **Error / parameter contracts (SHIPPED, scoped).** Every `fit` rejects
  ...
  (`_pls.py:203-212`) raising `ValueError`, NOT `FerroError`; sklearn's NIPALS bound is
  `rank_upper_bound = p` for regression (`:294`) while ferrolearn caps the three NIPALS
  estimators at `min(p,q,n)` (a TIGHTER bound); sklearn does not pre-reject
  `n_samples < 2`.
...
- REQ-9: **`PLSRegression::predict` / `coefficients_` element-wise value parity
  ...
  y_std).T / x_std` (`:399-400`). ferrolearn's `coefficients_ = W(PᵀW)⁻¹Qᵀ`
  (`PLSRegression::fit`) is the internal pre-scale `coef_`; `FittedPLSRegression::predict`
  centres+scales X, multiplies by `coefficients_`, then un-scales by `y_std` and adds
  `y_mean` — algebraically equal to sklearn. Re-audit on the fresh fixture matches
  `predict` at 8.9e-16 (Probe 1: row0 `[1.50429937, 0.81156907, 2.40767472]`).
...
- AC-2 (REQ-4, SHIPPED): every `fit` `Err`s on row mismatch, `n_components==0`,
  ...
  mismatch. FLAG: sklearn raises `ValueError` and uses `rank_upper_bound=p` for regression
  (ferrolearn's bound is the tighter `min(p,q,n)`).
...
- AC-8 (REQ-17/18/19/20/21, NOT-STARTED): ferrolearn exposes no scaled/transposed `coef_`
  / `x_rotations_` / `y_rotations_` / `intercept_` accessor (#1623), no `deflation_mode`/
  `mode`/`algorithm='svd'` ctor params (#1624), no `inverse_transform` (#1625), no PyO3
  binding (#1626), and computes on `ndarray` + faer/Jacobi, not ferray (#1627).
...
SHIPPED-scoped), REQ-17 through REQ-21 are NOT-STARTED (5 rows). **Table totals: 16
SHIPPED / 5 NOT-STARTED.**
...
| REQ-4 (error / parameter contracts, scoped) | SHIPPED | every `fit` `Err`s on row mismatch (`ShapeMismatch`), `n_components==0` (`InvalidParameter`), `n_components>max` (`InvalidParameter`), `n_samples<2` (`InsufficientSamples`); `transform`/`predict` `Err` on column mismatch via `apply_centre_scale`. **FLAG (surface, not value):** sklearn validates via `_parameter_constraints` (`_pls.py:203-212`) raising `ValueError`; sklearn's regression bound is `rank_upper_bound=p` (`:294`), ferrolearn caps at `min(p,q,n)` (TIGHTER); sklearn doesn't pre-reject `n_samples<2`. Non-test consumer: re-export `lib.rs:79-81`. Verification: `test_*_invalid_zero_components`, `_too_many_components`, `_row_mismatch`, `_insufficient_samples`, `_transform_shape_mismatch` PASS. |
...
| REQ-9 (`PLSRegression::predict` / `coefficients_` value parity) | SHIPPED | sklearn `predict` (`_pls.py:530-531`) centres X (no `/x_std`), `Ypred = X @ coef_.T + intercept_`, `coef_ = (x_rotations_@y_loadings_.T * y_std).T / x_std` (`:399-400`). ferrolearn `coefficients_ = W(PᵀW)⁻¹Qᵀ` (`PLSRegression::fit`) is the internal pre-scale coef; `FittedPLSRegression::predict` centres+scales X, ×`coefficients_`, ×`y_std`, +`y_mean` — algebraically equal. Re-audit matches at 8.9e-16 (Probe 1 row0 `[1.50429937, 0.81156907, 2.40767472]`). Non-test consumer: re-export `lib.rs:79-81`. Verification: `tests/divergence_cross_decomposition.rs` PLSRegression predict-parity test (live, green). |
...
| REQ-17 (`coef_`/`x_rotations_`/`y_rotations_`/`intercept_` attrs) | NOT-STARTED | open prereq blocker **#1623**. sklearn exposes `coef_ (n_targets,n_features)` × `y_std/x_std` (`_pls.py:399-400`), `intercept_=y_mean` (`:401`), `x_rotations_` (`:391`), `y_rotations_` (`:395`). `FittedPLSRegression::coefficients()` returns the INTERNAL pre-scale `(n_features,n_targets)` matrix (NOT transposed/scaled), and there is NO `coef_`/`x_rotations_`/`y_rotations_`/`intercept_` accessor (rotation recomputed inline in `transform`); canonical/CCA fitted structs expose neither. |
...
sign propagates uniformly. `PLSRegression::fit` additionally builds `coefficients_ =
W(PᵀW)⁻¹Qᵀ` via `invert_square` (REQ-9); `predict` reconciles the internal pre-scale
coefficient with sklearn's scaled/transposed `coef_` by centring+scaling X, multiplying,
then un-scaling by `y_std` and adding `y_mean`. `transform` (all three) recomputes the
...
`y_scores`/`n_iter` (NIPALS structs), `coefficients` (PLSRegression only), `transform_y`
(PLSSVD/PLSCanonical/CCA), and `x_weights`/`y_weights`/`x_mean`/`y_mean` (PLSSVD). There
is NO public `coef_`/`x_rotations_`/`y_rotations_`/`intercept_` accessor (REQ-17, #1623)
...
- **#1623** — REQ-17: public `coef_`/`x_rotations_`/`y_rotations_`/`intercept_` attribute
  accessors (currently only the internal pre-scale `coefficients()` on
  `FittedPLSRegression`; canonical/CCA expose neither; the `transform_y` Y-projection via
