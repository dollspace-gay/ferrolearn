...
| REQ-4 (`r2_score`) | NOT-STARTED | open prereq blocker #764. `pub fn r2_score` **returns `Err(NumericalInstability)` on constant `y_true`**; sklearn default `force_finite=True` returns 0.0 (imperfect) / 1.0 (perfect) (`_assemble_r2_explained_variance:866,877-891`). Pin: `r2_score([3,3,3],[3,3,2])` → sklearn **0.0**, ferro errors. No `force_finite`/`sample_weight`/`multioutput="variance_weighted"`. |
| REQ-5 (`mean_absolute_percentage_error`) | NOT-STARTED | open prereq blocker #765. `pub fn mean_absolute_percentage_error` **skips `y_true==0` samples and returns `+inf` if all zero**; sklearn divides by `max(|y_true|, eps)` (`:403-404`). Pin: `mape([100,0,200],[110,999,200])` → sklearn **≈1.4997e18**, ferro **0.05**. No `sample_weight`/`multioutput`. |
| REQ-6 (`explained_variance_score`) | NOT-STARTED | open prereq blocker #766. `pub fn explained_variance_score` **errors on constant `y_true`**; sklearn `force_finite=True` returns 0.0/1.0 (`:874-891,929`). Offset-invariance (mean-subtracted residuals) is correct. Pin: `evs([5,5,5],[1,2,3])` → sklearn **0.0**, ferro errors. No `force_finite`/`sample_weight`/`multioutput`. |
...
| REQ-17 (`d2_absolute_error_score`) | NOT-STARTED | open prereq blocker #777. `pub fn d2_absolute_error_score` = pinball `alpha=0.5`; **`<2` samples → `Ok(0.0)`**, sklearn **`nan`** (`:1766,1699-1702`). Pin: `d2_absolute_error_score([1.0],[1.0])` → sklearn **nan**, ferro **0.0**. No `sample_weight`/`multioutput`. |
