---
title: Phase 4 — Beyond Scikit-Learn
tags: [design-doc]
sources: []
...
- REQ-6: GPU-accelerated PCA via cuSOLVER's truncated SVD (CUDA) or custom WGSL compute shader (WGPU) ### Online/Streaming Learning
...
- REQ-11: Streaming data adapter that reads from an iterator of batches and feeds them to `PartialFit` models ### Calibration and Semi-Supervised
...
- REQ-14: `SelfTrainingClassifier` meta-estimator that iteratively assigns pseudo-labels to unlabeled data and retrains ### Column Transformer
...
- REQ-17: `ColumnTransformer` integration with the typed pipeline for compile-time column type checking when input schema is known ### Remaining Algorithms
...
- REQ-24: Factor Analysis, ICA, Dictionary Learning in `ferrolearn-decomp` ### Formal Verification
...
- REQ-27: Document the compile-time guarantee (unfitted model cannot call `predict`) as a formal proof carried by Rust's type system ### Performance
...
Prusti verification is attempted on the latest compatible nightly. Since Prusti typically lags behind stable Rust by 6-12 months, the verification CI job is allowed to be "informational" (non-blocking) until the toolchain stabilizes for our MSRV.
