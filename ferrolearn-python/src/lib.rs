...
fn _ferrolearn_rs(m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    m.add_class::<extras::RsLars>()?;
    m.add_class::<extras::RsLassoLars>()?;
    m.add_class::<extras::RsRANSACRegressor>()?;
    ...
    // Extras — preprocess
    m.add_class::<extras::RsBinarizer>()?;
    ...
    m.add_class::<extras::RsOrdinalEncoder>()?;
    m.add_class::<extras::RsLabelEncoder>()?;
