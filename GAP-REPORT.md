...
- Test evidence: the current tree contains 358 `tests/divergence_*.rs` files.
...
- Scoped ML-facing sklearn API gaps: 23 exact public items missing across the
...
- Whole sklearn infrastructure areas are not counted in that exact gap count: callbacks,
  ...
  full display/plotting behavior, metadata routing, and Python object compatibility.
...
`sklearn.tree`, `sklearn.metrics`, `sklearn.model_selection`, `sklearn.calibration`, `sklearn.inspection`,
...
`sklearn.kernel_approximation`, `sklearn.gaussian_process`,
`sklearn.gaussian_process.kernels`, `sklearn.covariance`,
`sklearn.neural_network`, `sklearn.dummy`, `sklearn.multiclass`,
`sklearn.multioutput`, and `sklearn.isotonic`. This means
...
- Display and plotting APIs remain narrower than sklearn even where exact
  display names now exist across metrics/model-selection/calibration/inspection:
  Rust display helpers return renderer-neutral data, not matplotlib artists,
  Python axes, or full `from_estimator` classmethod behavior.
...
| Tree / ensemble | Stacking remains absent. Tree `export_graphviz` and `plot_tree` now exist as Rust-native helpers over ferrolearn's flat `Node` storage, but still differ from sklearn around matplotlib artist returns, file-handle output, exact color gradients, impurity labels, and full `tree_.value` storage. Implementations still differ in AdaBoost decision/probability/SAMME.R/base-estimator behavior, Voting's heterogeneous estimator/weight/transform surface, random and missing-value routing details, HGB bin threshold and missing-direction behavior, RNG exactness, and broader sklearn visualization/export attributes. |
| Neighbors | The k-neighbor and radius-neighbor graph transformer names now exist, and `KernelDensity` has a dense Euclidean Gaussian scoring path pinned to sklearn log-likelihood values. `NeighborhoodComponentsAnalysis` remains missing. Present estimators still have gaps around `X=None` self-query behavior, `sort_results`, `include_self`, constructor surfaces (`radius`, `metric`, `p`, `metric_params`, `n_jobs`), KDE sample weights/non-Gaussian kernels/string bandwidths/tree tolerance controls/sampling, sparse/precomputed distances, exact error messages, and some tie/order behavior. |
...
| Model selection / compose / calibration / multiclass / multioutput | Main display class names now exist as renderer-neutral data containers, and `calibration_curve` covers scoped binary uniform/quantile binning. Several implementations use Rust closures or simplified wrappers rather than sklearn's estimator-cloning protocol. RNG exactness, result-table parity, display plotting/`from_estimator` behavior, threshold estimator protocol, calibration sample weights/multiclass defaults, and some group/splitter edge semantics remain documented divergence areas. |
...
| `ferrolearn-decomp` | 41 |
| `ferrolearn-kernel` | 13 |
| `ferrolearn-linear` | 70 |
| `ferrolearn-metrics` | 17 |
| `ferrolearn-model-sel` | 33 |
