...
impl<T> CooMatrix<T>
where
    T: Clone + Add<Output = T> + sprs::MulAcc + Zero + Default + Send + Sync + 'static,
{
    /// Matrix-matrix product `A @ B`, returning a [`CsrMatrix`].
    ///
    /// Mirrors scipy `coo_matrix.dot(other)` (`scipy/sparse/_base.py:587`, which
    /// dispatches to `self @ other` → `_matmul_dispatch` → `_matmul_sparse`,
    /// `:898`). COO has **no native matmul**: scipy's `_matmul_sparse`
    /// (`scipy/sparse/_base.py:898`) routes through `self.tocsr()._matmul_sparse(other)`,
    /// and arithmetic on `_spbase` defaults to CSR (the class comment at
    /// `_base.py:484` — "All arithmetic operations use csr_matrix by default"). So
    /// a COO `@` COO product **returns a CSR result**, not a COO. This method
    /// reproduces that exactly: it converts both operands to CSR
    /// ([`to_csr`](Self::to_csr), summing duplicates) and delegates to
    /// [`CsrMatrix::matmul`], whose result is the `(self.n_rows, rhs.n_cols)`
    /// CSR sparse-sparse product. `self.n_cols()` must equal `rhs.n_rows()`.
    ///
    /// Live oracle (R-CHAR-3): for `A = [[1,2],[0,3]]`, `B = [[4,0],[5,6]]`,
    /// `A.dot(B).toarray() == [[14, 12], [15, 18]]`; for the non-square
    /// `D = [[1,0,2],[3,4,0]]` (2×3), `E = [[0,5],[6,0],[0,7]]` (3×2),
    /// `D.dot(E).toarray() == [[0, 19], [24, 15]]` (2×2).
    ///
    /// # Errors
    ///
    /// Returns [`FerroError::ShapeMismatch`] (propagated from
    /// [`CsrMatrix::matmul`]) if `self.n_cols() != rhs.n_rows()` (scipy raises
    /// `ValueError: dimension mismatch`), or any [`FerroError`] from the COO→CSR
    /// conversion (infallible for structurally valid inputs).
    pub fn dot(&self, rhs: &CooMatrix<T>) -> Result<CsrMatrix<T>, FerroError> {
        self.to_csr()?.matmul(&rhs.to_csr()?)
    }
}

impl<T> CooMatrix<T>
where
    T: Clone + Add<Output = T> + Zero + 'static,
    for<'r> &'r T: Mul<&'r T, Output = T>,
{
    /// Element-wise (Hadamard) product `A * B`, returning a [`CsrMatrix`].
    ///
    /// Mirrors scipy `coo_matrix.multiply(other)` (`scipy/sparse/_base.py:490`).
    /// COO has **no native element-wise multiply**: scipy's `_spbase.multiply`
    /// routes a sparse operand through `self.tocsr()` and the `_elmul_` binary op
    /// (arithmetic on `_spbase` defaults to CSR — the class comment at
    /// `_base.py:484`, "All arithmetic operations use csr_matrix by default"). So a
    /// COO `multiply` COO product **returns a CSR result**, not a COO. This method
    /// reproduces that exactly: it converts both operands to CSR
    /// ([`to_csr`](Self::to_csr), summing duplicates) and delegates to
    /// [`CsrMatrix::multiply`], whose result keeps only positions stored (non-zero)
    /// in BOTH operands — the INTERSECTION sparsity, since the product is zero
    /// wherever either factor is a structural zero. `self.shape()` must equal
    /// `rhs.shape()`.
    ///
    /// Live oracle (R-CHAR-3): for `A = [[1,2],[0,3]]`, `B = [[4,0],[5,6]]`,
    /// `A.multiply(B).toarray() == [[4, 0], [0, 18]]` (only the overlapping
    /// `(0,0)=1*4` and `(1,1)=3*6` survive); for `D = [[2,3],[4,0]]`,
    /// `E = [[0,5],[6,7]]`, `D.multiply(E).toarray() == [[0, 15], [24, 0]]`
    /// (overlaps `(0,1)=3*5` and `(1,0)=4*6`).
    ///
    /// # Errors
    ///
    /// Returns [`FerroError::ShapeMismatch`] (propagated from
    /// [`CsrMatrix::multiply`]) if `self.shape() != rhs.shape()` (scipy raises
    /// `ValueError: inconsistent shapes`), or any [`FerroError`] from the COO→CSR
    /// conversion (infallible for structurally valid inputs).
    pub fn multiply(&self, rhs: &CooMatrix<T>) -> Result<CsrMatrix<T>, FerroError> {
        self.to_csr()?.multiply(&rhs.to_csr()?)
    }
}

impl<T> CooMatrix<T>
where
    T: Clone + Add<Output = T> + Zero + Default + 'static,
    for<'r> &'r T: Add<&'r T, Output = T>,
{
    /// Element-wise addition `A + B`, returning a [`CsrMatrix`].
    ///
    /// Mirrors scipy `coo_matrix + coo_matrix` (`scipy/sparse/_base.py:740`
    /// `__add__` → `_add_sparse`, `:724`). COO has **no native sparse addition**:
    /// scipy's `_spbase._add_sparse` routes through `self.tocsr()._add_sparse(other)`
    /// (arithmetic on `_spbase` defaults to CSR — the class comment at `_base.py:484`,
    /// "All arithmetic operations use csr_matrix by default"). So a COO `+` COO sum
    /// **returns a CSR result**, not a COO. This method reproduces that exactly: it
    /// converts both operands to CSR ([`to_csr`](Self::to_csr), summing duplicate
    /// `(row, col)` entries) and delegates to [`CsrMatrix::add`], whose result has the
    /// UNION sparsity of `A` and `B`. `self.shape()` must equal `rhs.shape()`.
    ///
    /// Live oracle (R-CHAR-3): for `A = [[1,2],[0,3]]`, `B = [[4,0],[5,6]]`,
    /// `(A + B).toarray() == [[5, 2], [5, 9]]`.
    ///
    /// # Errors
    ///
    /// Returns [`FerroError::ShapeMismatch`] (propagated from [`CsrMatrix::add`]) if
    /// `self.shape() != rhs.shape()` (scipy raises `ValueError: inconsistent shapes`),
    /// or any [`FerroError`] from the COO→CSR conversion (infallible for structurally
    /// valid inputs).
    pub fn add(&self, rhs: &CooMatrix<T>) -> Result<CsrMatrix<T>, FerroError> {
        self.to_csr()?.add(&rhs.to_csr()?)
    }
}
...
{
    /// Element-wise subtraction `A - B`, returning a [`CsrMatrix`].
    ///
    /// Mirrors scipy `coo_matrix - coo_matrix` (`scipy/sparse/_base.py:760`
    /// `__sub__` → `_sub_sparse`, `:730`). COO has **no native sparse subtraction**:
    /// scipy's `_spbase._sub_sparse` routes through `self.tocsr()._sub_sparse(other)`
    /// (arithmetic on `_spbase` defaults to CSR — the class comment at `_base.py:484`,
    /// "All arithmetic operations use csr_matrix by default"). So a COO `-` COO
    /// difference **returns a CSR result**, not a COO. This method reproduces that
    /// exactly: it converts both operands to CSR ([`to_csr`](Self::to_csr), summing
    /// duplicate `(row, col)` entries) and delegates to [`CsrMatrix::sub`], whose
    /// result has the UNION sparsity of `A` and `B`. `self.shape()` must equal
    /// `rhs.shape()`.
    ///
    /// Live oracle (R-CHAR-3): for `A = [[1,2],[0,3]]`, `B = [[4,0],[5,6]]`,
    /// `(A - B).toarray() == [[-3, 2], [-5, -3]]`.
    ...
    /// # Errors
    ///
    /// Returns [`FerroError::ShapeMismatch`] (propagated from [`CsrMatrix::sub`]) if
    /// `self.shape() != rhs.shape()` (scipy raises `ValueError: inconsistent shapes`),
    /// or any [`FerroError`] from the COO→CSR conversion (infallible for structurally
    /// valid inputs).
    pub fn sub(&self, rhs: &CooMatrix<T>) -> Result<CsrMatrix<T>, FerroError> {
        self.to_csr()?.sub(&rhs.to_csr()?)
    }
}
    ...
    T: num_traits::Float,
{
    ...
    ///
    /// Mirrors scipy `coo_matrix.power(n)` (`scipy/sparse/_data.py:99` —
    /// `return self._with_data(data ** n)`), which performs an **element-wise
    /// power over `self.data` only**, leaving the row/col index arrays, the
    /// `(n_rows, n_cols)` shape, and the stored count (`nnz`) unchanged. The
    /// implicit zeros stay zero because `0 ** n == 0` for `n > 0` (scipy refuses
    /// `n == 0`, which would densify the matrix; this method does not special-case
    /// it — `0.powf(0) == 1` per IEEE, matching `T::powf`). Each stored value is
    /// mapped through [`num_traits::Float::powf`].
    ///
    /// Duplicate `(row, col)` entries and explicit stored zeros are preserved (no
    /// coalescing) — only the data array is transformed.
    ///
    /// Live oracle (R-CHAR-3): for
    /// `sp.coo_matrix(([2.,-3.,4.],([0,1,2],[0,1,2])),shape=(3,3))`,
    /// `power(2)` yields `data == [4, 9, 16]` with `row == [0,1,2]`,
    /// `col == [0,1,2]`, `nnz == 3`, dense `[[4,0,0],[0,9,0],[0,0,16]]`;
    /// `power(3)` yields `data == [8, -27, 64]`. For
    /// `sp.coo_matrix(([4.,9.,16.],([0,1,2],[0,1,2])),shape=(3,3))`,
    /// `power(0.5)` yields `data == [2, 3, 4]`.
    ///
    /// # Errors
    ///
    /// Returns [`FerroError`] propagated from [`from_triplets`](Self::from_triplets);
    /// infallible for any structurally valid `CooMatrix` (the unchanged indices stay
    /// in bounds of the unchanged shape).
    pub fn power(&self, n: T) -> Result<CooMatrix<T>, FerroError> {
