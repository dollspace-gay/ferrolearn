...
fn conformance_nmf() {
    ...
    // The fixture's `reconstruction_error` was generated by sklearn's DEFAULT
    // `solver='cd'` (coordinate descent) with `init='nndsvd'` — sklearn itself
    // WARNS that `mu` + `nndsvd` "leads to poorer results" (it cannot update the
    // zeros NNDSVD seeds), landing at 5.5771 vs cd's 5.1846. Pair the conformance
    // run with `CoordinateDescent` so it matches the fixture's own generation
    // path; with the real SVD-based NNDSVD init (#2398) ferrolearn's cd now
    // reproduces sklearn's cd `reconstruction_err_` to ~1e-9 here.
    let nmf = ferrolearn_decomp::NMF::<f64>::new(n_components)
        .with_init(init)
        .with_solver(ferrolearn_decomp::NMFSolver::CoordinateDescent)
