...
const MATERN_ORACLE: &[(f64, f64, [f64; 16])] = &[
    (0.7, 0.5, [
        1.0, 0.13828069713920702, 0.13828069713920702, 0.004659343044852556,
        0.13828069713920702, 1.0, 0.05499154106561043, 0.012699132490841826,
        0.13828069713920702, 0.05499154106561043, 1.0, 0.022874378205733976,
        0.004659343044852556, 0.012699132490841826, 0.022874378205733976, 1.0,
    ]),
    (0.7, 1.0, [
        1.0, 0.406181840375756, 0.406181840375756, 0.07941967122077294,
        0.406181840375756, 1.0, 0.26180486407843745, 0.12920677700694147,
        0.406181840375756, 0.26180486407843745, 1.0, 0.17171828903750483,
        0.07941967122077294, 0.12920677700694147, 0.17171828903750483, 1.0,
    ]),
    (0.7, 2.3, [
        1.0, 0.7148576610326355, 0.7148576610326355, 0.37084097727436705,
        0.7148576610326355, 1.0, 0.6010123302913372, 0.45227479736673865,
        0.7148576610326355, 0.6010123302913372, 1.0, 0.5074706912545756,
        0.37084097727436705, 0.45227479736673865, 0.5074706912545756, 1.0,
    ]),
    (3.5, 0.5, [
        1.0, 0.13778061855662008, 0.13778061855662008, 0.0004289724370097413,
        0.13778061855662008, 1.0, 0.03308036271565073, 0.002672462003681965,
        0.13778061855662008, 0.03308036271565073, 1.0, 0.007542311542953075,
        0.0004289724370097413, 0.002672462003681965, 0.007542311542953075, 1.0,
    ]),
    (3.5, 1.0, [
        1.0, 0.5449424471128748, 0.5449424471128748, 0.05954656958951078,
        0.5449424471128748, 1.0, 0.3280670124332057, 0.12478772930042893,
        0.5449424471128748, 0.3280670124332057, 1.0, 0.18750506688966975,
        0.05954656958951078, 0.12478772930042893, 0.18750506688966975, 1.0,
    ]),
    (3.5, 2.3, [
        1.0, 0.8803125953553694, 0.8803125953553694, 0.4943397494173103,
        0.8803125953553694, 1.0, 0.7808056269023986, 0.6076812434745492,
        0.8803125953553694, 0.7808056269023986, 1.0, 0.6773949221824243,
        0.4943397494173103, 0.6076812434745492, 0.6773949221824243, 1.0,
    ]),
    (5.5, 0.5, [
        1.0, 0.13676937915110277, 0.13676937915110277, 0.00019936427430053568,
        0.13676937915110277, 1.0, 0.028755640601731965, 0.001661041834744783,
        0.13676937915110277, 0.028755640601731965, 1.0, 0.005444355498132258,
        0.00019936427430053568, 0.001661041834744783, 0.005444355498132258, 1.0,
    ]),
    (5.5, 1.0, [
        1.0, 0.5660530869645884, 0.5660530869645884, 0.05505723350068352,
        0.5660530869645884, 1.0, 0.3399358057083622, 0.12299252787543835,
        0.5660530869645884, 0.3399358057083622, 1.0, 0.18980576305684238,
        0.05505723350068352, 0.12299252787543835, 0.18980576305684238, 1.0,
    ]),
    (5.5, 2.3, [
        1.0, 0.892507575042342, 0.892507575042342, 0.5141601134322807,
        0.892507575042342, 1.0, 0.7991683256346874, 0.6295329487542534,
        0.892507575042342, 0.7991683256346874, 1.0, 0.6988669064582632,
        0.5141601134322807, 0.6295329487542534, 0.6988669064582632, 1.0,
    ]),
];
...
fn gpr_matern_nu_07_predict_std_matches_sklearn() {
    ...
    for (i, &e) in ORACLE_MEAN.iter().enumerate() {
        assert!((mean[i] - e).abs() < 1e-6, "nu=0.7 mean[{i}]: ferro={}, sklearn={e}", mean[i]);
    }
    for (i, &e) in ORACLE_STD.iter().enumerate() {
        assert!((std[i] - e).abs() < 1e-6, "nu=0.7 std[{i}]: ferro={}, sklearn={e}", std[i]);
    }
}
...
fn gpr_matern_nu_55_predict_std_matches_sklearn() {
    ...
    let (mean, std) = fitted.predict_with_std(&x_query()).unwrap();
    for (i, &e) in ORACLE_MEAN.iter().enumerate() {
        assert!((mean[i] - e).abs() < 1e-6, "nu=5.5 mean[{i}]: ferro={}, sklearn={e}", mean[i]);
    }
    for (i, &e) in ORACLE_STD.iter().enumerate() {
        assert!((std[i] - e).abs() < 1e-6, "nu=5.5 std[{i}]: ferro={}, sklearn={e}", std[i]);
    }
}
